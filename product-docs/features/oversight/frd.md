...
| OV-003 | Must | Invitation surface for admins: create (email + target org), pending/sent list scoped to the caller's org, revoke, declined shown as status. Register form carries the invitation token end-to-end. Its delivery half is OV-003b, which is the spelling the code cites. |
| OV-003b | Must | Invitation delivery: creating an invitation sends it, as one transactional email to the invited address carrying the token-bound register link. A refused send never fails the create — the admin reads that the invitation exists and the email did not go, and every pending row offers **Resend email**. No surface renders an invitation link for an admin to copy and pass on by hand. *Why:* the admin's copy of the link addresses nobody, so it can be handed to the wrong person; the email addresses exactly the invitee. |
...
| OV-007 | Must | Disassociation from both sides: planter leaves from the settings area; admin removes a plant from the plant detail page. Both behind type-to-confirm; both notify the other side. For both, the dialog is a deliberateness control and never an authorization one — the OV-010 authority rule, the tenancy assertion, the OV-008 audit row and the notification all sit behind the action, so a request that never opened the dialog meets them too. The two sides are named below, which is the spelling the code cites. |
