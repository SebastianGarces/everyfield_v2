...
## 2026-08-13 — Factory rulings: the pre-release risk policy (#435), the per-site review-fix cap (#430, on PR #428) and comment provenance (on PR #432)

Three rulings about the delivery factory itself, all given by Sebastian on 2026-08-13. None of them
changes product behaviour; all three change what the loop is obliged to do, so they are recorded here
and applied to `ops/agent-os/dod.md`, `ops/agent-os/labels.md`, `ops/agent-os/delegation-rules.md`,
`.claude/skills/spec-intake/SKILL.md` and `.claude/skills/dispatch/SKILL.md` in the same change.

| # | Decision | Consequence |
|---|----------|-------------|
| 435 | **`risk:high` narrows to auth/permissions, multi-tenant isolation and payments. Schema and migrations are NOT high-risk pre-release.** There is no separate production database serving real client data, so a wrong migration costs a dev-DB reset, not a recovery — while the label's price (attended dispatch only, a three-lens HR4 pass, never an auto-merge) is priced for damage that cannot yet happen. **Revert condition: the moment alpha or beta serves real client data from a separate production database, schema/migrations return to `risk:high`.** That reversal takes a new ruling and a new row here — never a verifier's judgement call — and the condition is restated wherever the rule itself is stated. | Migration tracks dispatch unattended and may auto-merge like any other track. Applied on the board by hand the same day: `risk:high` dropped from #380 #324 #98 #89 #75 #70; kept on #308 #146 #22 #185 #311 (auth/tenancy), #62 (privacy/consent), #323 (HR4-hardening content), #190 (user data to an external GitHub), #378 (user↔person auth link), #320 (file-upload surface). |
| 435 (a) | **The migration proofs re-key off the label and onto the diff: HR1–HR3 fire whenever the track's diff carries a migration, at ANY risk tier.** HR4 (diverse-lens sign-off), attended-only dispatch and never-auto-merge stay keyed to `risk:high`. | The dry-run, the rollback proof and the DDL delta in the PR body survive the narrowing intact — a `risk:medium` schema track owes all three. The verifier's question becomes *does this diff contain a migration?*, never *what is this issue labelled?*. `dod.md`'s TRACK DONE line now reads `(+ HR1..HR3 if the diff carries a migration; + HR4 if high-risk)`. |
| 435 (b) | **A source comment states a constraint; provenance never appears in source.** Issue and PR numbers, ruling dates, review-round stamps and attempt counters live in the commit message, the PR body and `memory/` — the three places that are already dated, attributed and searchable. A comment earns its place only by saying what the code cannot show (why this order, which invariant it upholds, what breaks if it changes); citing the invariant or the test that pins it stays, because that is an enforcement pointer with a live address rather than a history entry. Test files are outside the rule. Ruled while reviewing PR #432, where agent-written code had begun narrating its own delivery history in the files it touched. | Lands as `ops/agent-os/delegation-rules.md` **R7** (the sixth rule becomes the seventh) and as a `dod.md` **G4** convention check over the track's added non-test source lines, with the grep a verifier runs rather than recalls. Prompt-level wiring — teaching the implementer prompts the same rule up front — is deliberately deferred to the planned workflow-simplification session, so today the rule is caught at review rather than prevented at write time. |
| 430 | **The 2-round review-fix cap counts PER REVIEW SITE.** The three sites — scoped review (per workstream), integration verify (G6), and any post-integration quality round on the assembled branch — each carry their own 2 rounds; rounds spent at one never subtract from another's budget. Ruled on PR #428. | `dod.md` states the reading explicitly instead of leaving it to be re-derived per pass. PR #428 and the U411-WIKI track both had to guess; under the rejected per-track reading a track that spent two scoped rounds would have to HOLD on its first integration finding, which is not what the loop does. The `dispatch` skill already read it this way and now cites the ruling. |
