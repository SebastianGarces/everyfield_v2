...
| 657 (2) | **The section read is a ROUTE HANDLER, not a server action** — `GET /api/settings/sections/<id>`. | Also measured, on the preview, and it is a correctness requirement rather than a style: every server-action response carries a fresh RSC render of the current route, which is how `refresh()` delivers an update. The modal re-reads its section when that render changes (that is what makes a write show up), so a read spelled as an action re-triggered itself on its own answer — one notification toggle produced an unbounded loop at about seven requests a second. A route handler re-renders nothing, so a write now costs exactly one extra read. Guarded by `settings-mechanism.test.ts`, because the two spellings look identical in a diff. |
