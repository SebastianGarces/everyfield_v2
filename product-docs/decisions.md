...
## 2026-08-21 — Plant Intelligence rubric v1 is in force (#538, closing the #469 series)

Ruled by Sebastian: **"we don't have any customers, flip the rubric to v1."** The #469 series
turned Bryan Nass's 26-comment review of rubric v0 into 18 ruled changes (#470–#487), each
landing its delta into a registered-but-inactive `RUBRIC_V1_BODY` behind a test that kept v0
running. #538 assembled them, fixed the seams, and flipped the switch.

| # | Decision | Consequence |
|---|----------|-------------|
| 538 | **`ACTIVE_RUBRIC_VERSION` is `"v1"`. Every assessment generated from 2026-08-21 records `rubric_version: v1`.** The D0 series decision (2026-08-20) deliberately deferred activation to one issue so 18 parallel PRs could each edit the judge's scoring rules without any one of them changing what a planter receives. The flip is the one line that does. **Why now rather than at alpha:** there are no customers, so there is no cohort whose historical assessments would suddenly be explained by a rubric they never saw. Flipping later would mean flipping *with* that cohort, which is strictly worse. | v0 stays registered and `getRubric("v0")` still resolves: assessments written before the flip recorded `v0` and are re-explained against the text that produced them, which is the whole reason the version is stored per assessment (AC-PE-4). `rubric-v0.md` is kept as the historical record, not deleted. `rubric-v1.md` loses its draft banner. The guard test inverts — it now asserts v1 is active AND v0 still resolves, which is the half of the contract that outlives the flip. |
| 538 (1) | **C12 — the vision-meeting target (biweekly vs weekly) — is DEFERRED, and the rubric says so in as many words.** It is the one comment of the 26 with no ruling. Bryan reported that he met weekly and consistently; v1 took the half that the data supports — **consistency** is now part of Lens 1's healthy definition — and did not move the target from 14 days to 7. **Three reasons it is not ruled:** the target is a *Launch Playbook* parameter, so changing it is a methodology decision rather than a rubric edit; one practitioner's practice is not evidence enough to move a methodology number (the exact failure C03 exists to prevent); and nothing is blocked by leaving it open. | Lens 1 assesses against every two weeks and carries an explicit line barring the judge from presenting biweekly as the only defensible rhythm, or from telling a planter who meets weekly that they are over-meeting. Recorded in `rubric-v1.md` (Lens 1 and Appendix C question 6) and in `bryan-comments-review.md`'s closing disposition. It is the standing question for Brett's review — a second practitioner is what would actually settle it. |
| 538 (2) | **The network verdict ban-list has ONE spelling, and the rubric interpolates it.** The coherence pass found the rubric naming 7 banned phrases while `NETWORK_VERDICT_PHRASES` failed the judge's output on 9 — so the judge could emit "must be addressed" or "at risk of failure", phrases it was never told about, and have the whole response rejected. A code ruling, recorded here because it is the pattern rather than the instance: a rule the model is *told* and a rule the model is *held to* may not be two hand-maintained lists. | `rubric.ts` imports `NETWORK_VERDICT_PHRASES` and interpolates it into v1's body; a test asserts every enforced phrase appears in the active rubric. Two further coherence guards ship with it — no duplicated heading and no duplicated substantive line in the active body — because the assembled v1 arrived carrying two verbatim duplications from independent sub-issues, and nothing was reading the document as a document. |

## 2026-08-21 — C12 ruled: the vision-meeting target stays biweekly

| # | Decision | Consequence |
|---|----------|-------------|
| C12 | **The vision-meeting target stays every two weeks.** Ruled by Sebastian the same day the deferral above was recorded, closing the last open comment of Bryan's 26. The Playbook's suggestion is the floor the engine assesses against. The nuance that decides it: Bryan met weekly **once he had a core team** — weekly is a rhythm a team grows into, not the Phase 1 floor, so measuring every plant against 7 days would nag exactly the early-stage plants the lens exists to serve. Both judge bans survive the ruling unchanged: the engine never presents biweekly as the only defensible rhythm, and never tells a planter meeting weekly that they are over-meeting. | `rubric-v1.md` Lens 1 and Appendix C question 6 convert from "open parameter" to this record; `bryan-comments-review.md`'s disposition moves C12 from DEFERRED to RULED — 26 of 26 closed. No code changes: `rubric.ts` already assesses against 14 days, and the 21/28 ladder (#486) was ruled in absolute days. Network-level configurability of the target remains a later product decision, as already noted in the rubric. |
## 2026-08-21 — The leadership claim needs no audit rail; ownership transfer is the flow that does (#342)

Ruled by Sebastian in the #342 discovery's lavish review. The issue asked whether the OB-010
claim path — a Member self-promoting to the Owner seat — should write an audit row and
notify. The discovery proved the question moot for alpha and renamed what #342 is for.

| # | Decision | Consequence |
|---|----------|-------------|
| 342 | **The claim path gets NO audit row and NO notification, because production can never reach it.** The verified chain: registration mints an Owner in every branch; a Member only ever joins a church that already has one; nothing empties the Owner seat (an Owner cannot remove their own, removal only touches other seats, transfer is out of scope); the claim requires a church with no Owner. Starting from an empty production database, the state the claim needs cannot arise — it stays reachable only on seeded/legacy dev rows and after manual support surgery, where it is the recovery path. Building the rail would guard a door that cannot open. | #342 leaves the Alpha milestone for Beta and becomes the **ownership-transfer discovery** — the code already names it ("Handing ownership over is #342's verb", the seat-roster Owner-removal guard). When transfer ships it must write a church-scoped append-only leadership event row (the launch-events shape, batched with the write it audits) and notify the plant's other seats and each associated org on the milestone rail — the drafted proposal and its open shape questions (narrow `leadership_events` vs broad `seat_events`; org notice consent-exempt vs toggle-gated; retroactivity; whether a decline also records) are carried in #342. The claim path code stays as-is: guarded, race-closed by the one-Owner index, security-lens-passed. |

## 2026-08-21 — Settings is a URL-driven modal, not a page (#187)

Ruled by Sebastian from the Claude settings modal as the reference (Codex settings share the
shape). Replaces the 2026-08-15 "one route with tabs" shape before any of it was built.

| # | Decision | Consequence |
|---|----------|-------------|
| 187 | **Settings opens as a MODAL over whatever screen the user is on — side navigation of sections, a search box, the selected section on the right — and the URL carries its state: `/settings/<section>` opens it, closing returns to the screen behind.** The reference keeps settings one gesture away from any screen instead of a page you navigate to and back from. Search ships as a Should (CS-016): with five sections it earns little, but it is part of the reference shape and grows with the section list. | Real paths, never a `#settings/...` hash — the server never sees a hash, so seat guards and server rendering would not hold. In-app navigation overlays the live screen; a direct load renders the same modal over the dashboard. `/settings/team` and `/settings/association` keep their URLs and render as sections inside the modal, so no link or email breaks. One settings registry — section id, label, icon, entry keywords, required capability — drives the side nav, the search, and which sections render for which account shape, so the section list is data, not per-page conditionals. |

## 2026-08-22 — "Unknown is not healthy" refuses the positive band, and only that band (#635)

Ruled in the #635 track under *Decide, don't ask* (`ops/process.md`), applying #483's C17 rule to
the one lens that escaped it. A cold-start plant's CSF-1 tile read "Vision casting · GOING WELL ·
Based on no activity recorded yet" while its seven siblings read "Insufficient evidence" off the
same absence. The question the issue left open was whether vision casting is deliberately allowed
to read positive early in Phase 0.

| # | Decision | Consequence |
|---|----------|-------------|
| 635 | **It is not. CSF-1 follows the same rule as the other seven: a lens with nothing measured and nothing attested may never carry a `positive` insight, for either audience.** The rule was already written — v1's evidence-quality section says an unknown lens produces "at most an insufficient-evidence statement… never a quiet pass, a blank, or an encouraging remark" — and it was written in prose only, which is why seven lenses obeyed it and the eighth did not on the one plant with nothing else to say. It binds the network audience too: `buildCsfScorecard` projects the overseer's insights onto the same eight tiles with the same vocabulary, and an overseer acting on a false pass is the more expensive error of the two. | `unknown_lens_not_healthy` is a refinement on the judge's per-plant schema (`judgeOutputSchemaFor`), so a draft breaking it is re-prompted rather than stored, and the schema is now a function of the plant's evidence profile — there is no snapshot-free judge schema. The three prompt passages that offered "pair with a `positive` insight, it's free" now name the blind lenses where that device is unavailable. `buildCsfScorecard` drops a pass a blind lens never earned, so assessments stored before the rule render the insufficient-evidence tile rather than the verdict. |
| 635 (1) | **The NEUTRAL band stays available on a blind lens.** An `info` insight — Bryan's "we don't have enough information to assess prayer yet", or a plain statement of the absence with the first step beside it ("vision meetings have not yet commenced; consider setting a rhythm") — claims nothing about health and is what a cold-start planter is owed (PE-018). Only `positive` is a verdict, so only `positive` is refused. Forbidding the neutral band would restore the blank #483 exists to remove, and the issue itself ruled a neutral band derived from an absence defensible when it excluded CSF-7's "Noted" from the report. | A cold-start tile reads "Insufficient evidence" when the judge says nothing about the lens, and "Noted" when it names the absence and points at the first step. Neither says the plant is doing well. The fleet audit (`scripts/run-eval-v1-pass.ts`) holds `watch`/`urgent` on a blind lens to the insufficient-evidence phrasing and lets `info` through, and it now asks this of all eight lenses — it asked only prayer and generosity, which is why the fleet pass reported no violations while the bug was live. |

## 2026-08-22 — The hide-everywhere sweep narrows two checklist rows, and finds a wall behind eleven (#499)

Ruled in the #499 track under *Decide, don't ask* (`ops/process.md`). AS-020's read-only surface
checklist was written as a list of controls to hide. Walking it row by row against the capability
table (#498) found two rows whose wording asks for more than the server does, and one structural
fact that changes what the other rows are even claiming.

| # | Decision | Consequence |
|---|----------|-------------|
| 499 | **A coach and an org Member never render rows 2–13 at all, so those rows are about the plant MEMBER.** Both carry `church_id = null`, and every plant surface opens with `verifySession()` then `if (!user.churchId) redirect(…)`. The coaching reach is one aggregate page, `/coaching/[churchId]`, which renders no control. The checklist reads as though three contexts walk twelve screens each; two of them are refused at the door. | The proof moves from twelve hidden buttons to one asserted wall: `read-only-surfaces.test.ts` walks the `(dashboard)` route tree and fails on a plant route that does not refuse a churchless account, so the property holds for routes nobody has written yet. A new plant surface must either carry the guard or be named, with its reason, in `NOT_PLANT_SURFACES`. |
| 499 (1) | **Ticking a launch milestone STAYS for a plant Member**, against the row's wording ("edit milestones" must not render). LS-007 split `launch.milestone` (SEATED) from `launch.schedule` (OWNER_ONLY) deliberately, so milestone completion follows ordinary task rules. Hiding it would refuse in the UI what the server allows. **An over-hide is the same drift as an under-hide** — a control missing from somebody the server would have said yes to is as wrong as a control beside an action guaranteed to refuse — and it is the harder one to notice, because nobody files a bug about a button they never knew existed. | `MilestoneBoard`'s `canEdit` was hardcoded `true`; it is now `holdsSeatFor(user, "launch.milestone")`, which admits the Member and refuses a coach and an org account. The same reasoning keeps every `read` and `self.write` control visible in a read-only context: CSV export, document generation and downloads, print, bookmarks, reading progress, notification preferences, insight and product feedback. Every render test in the sweep asserts BOTH directions for this reason. |
| 499 (2) | **The checklist's team-leader exception cannot ship, and no surface may fake it.** The row promises "a team leader's writes on their own team stay, derived from `MinistryTeam.leader_id`". `leader_id` references `persons.id`; a session names a `users.id`; nothing joins them until AS-013's registration link lands. `team.leaderId === user.id` compares two id spaces and is false for everyone for ever — a gate that looks like a feature and is a latent bug. | Every Ministry Teams write is hidden for a Member, which matches what the server does: the same missing link is why all of them sit at `teams.write` (ADMIN_PLUS) rather than at an own-duty verb. Recorded as an accepted residual in `memory/invariants.md`, retired by the account-to-person link, which restores the server half and the render half together. The FRD's other two exceptions are unaffected — a Member's own assigned task keeps its complete control (`tasks.own`, gated on `assignedToId`), and a Member's own RSVP is answered at `/rsvp/[token]`, outside the dashboard and outside a session. |
| 499 (3) | **The checklist itself becomes a table a test walks**, rather than a list somebody attests to having walked. | `READ_ONLY_SURFACE_CHECKLIST` (`@/lib/auth/read-only-surfaces`) carries the FRD's 22 rows with both columns verbatim; the suite parses the FRD's markdown and compares row for row, so neither side can be edited alone. A `fixed-here` row must name the files it changed, and each is asserted to still ask for the row's verb. The FRD text is unchanged by this ruling — the two narrowings above are recorded on the code table's rows and here, because rewriting a requirement to match an implementation is the one direction this ledger does not travel. |

## 2026-08-22 — Settings becomes a fragment, and the settings routes retire (#657)

Ruled by Sebastian after reviewing the shipped modal. Navigating between sections went through
the settings routes, so the modal reloaded and re-animated on every switch. This supersedes the
"real paths, never a `#settings/...` hash" half of the 2026-08-21 §187 row above; the rest of
that row — the modal shape, the side navigation, the search box, the one registry — stands.

| # | Decision | Consequence |
|---|----------|-------------|
| 657 | **The hash IS the state. `#settings/<section>` opens settings over whatever page the reader is on — `/meetings#settings/account`, `/phase#settings/church` — and changing sections rewrites the fragment and nothing else: no route change, no remount, no re-animation. Closing clears the fragment and the page behind never moved. A cold load with the fragment renders the page normally and opens the modal over it. The `/settings/*` routes are DELETED**, with thin permanent redirects for the URLs already in mail and bookmarks (`/settings/<section>` → `/dashboard#settings/<section>`, covering `NOTIFICATION_PREFERENCES_PATH`, `/settings/team`, `/settings/association` and the retired `/settings/sharing`). | The 2026-08-21 row's reason for real paths was that the server must see the section to render it and to gate it. That reason is answered rather than ignored: **section content becomes client-rendered inside the modal, and its data loads through one server endpoint (`loadSettingsSection`) which mints the session first and asks the same registry gate the navigation is built from.** Seats are still enforced server-side, on the read and on every write — UI gating stays presentation, enforcement stays in the actions. The registry (`src/lib/settings/sections.ts`) remains the single section list and now owns the fragment grammar too. |
| 657 (1) | **A server function returning the section's JSX was tried and does not work on this stack**, which is why the sections became client components rather than staying server components loaded on demand. | Measured on Next 16.2.2 with Turbopack, on both a static and a `force-dynamic` route: a server function CAN return a server-rendered tree, but a tree containing any client component fails to serialize — `Could not find the module "…" in the React Client Manifest`. Every settings section is mostly client components, so the option was closed by the platform, not by preference. Recorded here because the next reader will otherwise propose it again. |
| 657 (2) | **Three mechanism corrections, all found by driving the built thing rather than by reading it**, and each is recorded because the wrong version of it looks perfectly reasonable in a diff. | (a) The section read is a ROUTE HANDLER, not a server action: every server-action response carries a fresh RSC render of the current route, which is how `refresh()` delivers an update — and the modal re-reads its section when that render changes, so a read spelled as an action asked for itself again on its own answer, without end. (b) The in-flight read is cached in module scope rather than a `useMemo`: a render that suspends is discarded and replayed, and a replay re-runs the memo and mints a second promise, which suspends again — a loop at about six reads a second. (c) The modal opens through `pushState`, never a native `#`-navigation, because Next's router does not observe one: its stale copy of the URL was written back on the first `refresh()`, closing the modal on the reader's first save. All three are now guarded by `settings-mechanism.test.ts`, each assertion shown red. |
| 657 (3) | **The review found four more of the same kind, so the history policy moved out of the component into a module a test can DRIVE** (`src/lib/settings/settings-hash.ts`, `settings-hash.test.ts`). | Every guard written for this mechanism up to that point read source text — one mount, no settings page, no raw href — and all of them passed while these sat in the code: Close walked a reader off the page when the arrival changed page and fragment together (which `verify-email/confirmed` does); a read outlived the visit, so reopening a section showed the previous visit's rows; a failed read was cached under a key nothing moved, so Retry replayed it; and a 401 read as a refusal drew a skeleton that never resolved. A topological guard cannot see timing, and the lesson is recorded as the rule rather than as four fixes: the policy keeps a behavioural test. |
| 657 (3) | **The search box is no longer focused when the modal opens.** It still filters the section list and Enter still jumps to the first match. | Opening settings put a cursor in a field nobody asked for, and a screen reader announced "Search settings, edit text" instead of the dialog. Focus goes to the dialog itself, so its title is announced, Escape works, and Tab starts at the top of the rail. |

## 2026-08-25 — The authenticated app restores shadcn geometry (#688)

Ruled by Sebastian for the authenticated-shell refactor. This supersedes only the
2026-07-31 rectangle discipline as applied to the app; the marketing direction is
unchanged.
