...
| `repro-first` | **The default for every `bug`-labeled unit — `risk:high` ones INCLUDED** (ruled 2026-08-13 on PR #420); set it on those without being asked, and only depart from it with a reason in the dispatch comment. Rationale for winning that collision: HR4 still backstops security at integration whatever recipe ran, and enforced red→green has no backstop. Also earns its place on any unit whose acceptance criteria describe an observed misbehaviour (a crash, a wrong number, a flake) rather than a new capability, whatever its label. One agent writes the failing repro FIRST and RUNS it — **on attempt 1** a repro that never goes red refuses the attempt before an implementer is spent; a RETRY does NOT refuse, it warns, hands the implementer the verifier's named finding in the repro's place, and reports the repro unconfirmed — then the implementer fixes the code without touching that repro, and a third agent re-runs the same command to confirm it went green. Weighs **1** in `RECIPE_AGENT_COST`: three sequential agents, but the repro is test-writing the implementer would have done anyway, so an attempt costs about what `implement-straight` costs. Rationale: #307 and #315 each shipped a confident fix for a defect nobody had reproduced, and paid a full verify + review cycle to find out. |
| `adversarial-implement` | **The default for every `risk:high` unit that is not ALSO `bug`-labeled** — set it on those without being asked, and only depart from it with a reason in the dispatch comment; on a `risk:high` bug `repro-first` takes it (see the collision rule below). Also earns its place on any unit touching auth, tenancy, invitations, a `"use server"` export or a public route handler, whatever its risk label. An adversary attacks the diff in-worktree (forged inputs, tenant boundaries, SESSION-FIRST, injection, over-broad reads — the HR4 security lens, run PRE-gate) and the implementer fixes, looping until a round finds nothing new, capped at 3 rounds. Costs ~3× and weighs 3 in `RECIPE_AGENT_COST`; its agents are sequential, so the weight is about funding the whole loop, not about simultaneity. Rationale: #304 burned ~8 integration rounds on holes this pass catches for one commit each. |

- `recipe: "<id>"` is set **per unit** in the units array; omitted means `implement-straight`.
- Both label defaults are **selection** rules, not enforcement ones: the loop does not infer a
  recipe from `risk` or from a label, so a `risk:high` unit dispatched without
  `recipe: "adversarial-implement"` — or a `bug` unit without `recipe: "repro-first"` — simply runs
  straight. Dispatch is the only thing that sets it.
- When the two defaults collide (a `risk:high` bug), **`repro-first` wins** and the dispatch comment
  says so — INVERTED by ruling on 2026-08-13 (PR #420, decision 1b); the original call gave it to
  `adversarial-implement`. The two disciplines do not have the same fallback: a `risk:high` track
  meets **HR4's security lens at integration whatever recipe it ran**, so the adversary pass is
  backstopped, while enforced red→green has no backstop anywhere in the factory. The losing rule
  compensated with a sentence in the unit summary asking for a failing repro first, which is exactly
  the "request that costs nothing to skip and cannot be checked afterwards" that `repro-first`
  exists to replace with a machine. Say in the unit summary that the diff will be read for security
  holes at integration; write no prose stand-in for the repro, because there no longer is one.
  A composed repro → adversary recipe was the other option and is **deferred** until both canaries
  (#378, #398) have run.
