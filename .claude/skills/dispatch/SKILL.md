...
| `repro-first` | **The default for every `bug`-labeled unit** — set it on those without being asked, and only depart from it with a reason in the dispatch comment. Also earns its place on any unit whose acceptance criteria describe an observed misbehaviour (a crash, a wrong number, a flake) rather than a new capability, whatever its label. One agent writes the failing repro FIRST and RUNS it — a repro that never goes red refuses the attempt before an implementer is spent — then the implementer fixes the code without touching that repro, and a third agent re-runs the same command to confirm it went green. Weighs **1** in `RECIPE_AGENT_COST`: three sequential agents, but the repro is test-writing the implementer would have done anyway, so an attempt costs about what `implement-straight` costs. Rationale: #307 and #315 each shipped a confident fix for a defect nobody had reproduced, and paid a full verify + review cycle to find out. |
...
- When the two defaults collide (a `risk:high` bug), **`adversarial-implement` wins** and the
  dispatch comment says so. A hole that is reachable from outside costs more than an unproven fix
  does, and the adversary loop is the only one of the two that cannot be recovered later by reading
  the diff. Say in the unit summary that the fix must start from a failing repro; the implementer
  reads it either way.
