  ...
  async (t) => {
    ...
    try {
      ...
      const runProduction: ProductionEvalRunner = async ({
        ...
        const caseId = identityNotesFixtureIds.find((id) => id === scenario.id);
        assert.ok(caseId);
        assert.deepEqual(
          scenario.turns,
      ...
      const notesAdapter = createProductionEveEvalAdapter({
        store,
        buildSha: "0".repeat(40),
        runProduction,
      });
      const ambiguityAdapter = createProductionEveEvalAdapter({
        store,
        buildSha: "0".repeat(40),
      ...
      for (const caseId of identityNotesFixtureIds)
        await t.test(caseId, async (t) => {
          ...
          const fixture = await (
            caseId === "notes-05" ? notesAdapter : ambiguityAdapter
          ).prepare(scenario);
          assert.ok(fixture);
          try {
            const modes =
