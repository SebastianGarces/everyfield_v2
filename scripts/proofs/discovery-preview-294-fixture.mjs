...
const manifest = {
  ...
  restrictions:
    "Root may apply to the explicitly authorized existing development database after migration78, never production or customer data. Existing unrelated rows must remain untouched. No sessions are seeded. Successful signup and association responses/leave/sever send mail and remain blocked unless the SDK RESEND_BASE_URL is independently verified to target a root-owned capture-only service with external egress blocked. No production provider calls or approval-flag bypass are authorized.",
};
const sql = [
  ...
  `DO $$ BEGIN IF to_regclass('public.discovery_profiles') IS NULL OR to_regclass('public.leadership_versions') IS NULL THEN RAISE EXCEPTION 'schema through78 required'; END IF; END $$;`,
];
sql.push(
...
const wikiChecks = ["transfer", "associatedSeat", "emptySeat"].flatMap(
  (key) => [
    assertion(
      `(SELECT count(*) FROM wiki_progress WHERE user_id=${u(key)} AND article_slug=${q(wiki.slug)} AND status='completed' AND completed_at IS NOT NULL)=1`,
...
const postSafeChecks = [
  "-- ROOT READ ONLY. Run after transfer (sharing consent ON), associated-seat refusal and empty-seat acceptance.",
  assertion(
...
const postSafeOracle = postSafeChecks.join("\n") + "\n";
const journal = JSON.parse(
  readFileSync(resolve("src/db/migrations/meta/_journal.json"), "utf8")
);
const guard78 = journal.entries.find((entry) => entry.idx === 78);
assert.equal(guard78?.tag, "0078_leadership_version_guard");
const guardHash = createHash("sha256")
  .update(readFileSync(resolve(`src/db/migrations/${guard78.tag}.sql`)))
  .digest("hex");
const schemaOracle =
  [
    assertion(
      "to_regclass('public.discovery_profiles') IS NOT NULL AND to_regclass('public.leadership_versions') IS NOT NULL AND to_regclass('drizzle.__drizzle_migrations') IS NOT NULL",
      "schema: migration78 prerequisites"
    ),
    assertion(
      `EXISTS(SELECT 1 FROM drizzle.__drizzle_migrations WHERE hash=${q(guardHash)} AND created_at=${guard78.when})`,
      "schema: exact committed78 ledger hash/stamp"
    ),
    assertion(
      "EXISTS(SELECT 1 FROM information_schema.columns WHERE table_schema='public' AND table_name='church_privacy_settings' AND column_name='share_wiki') AND EXISTS(SELECT 1 FROM information_schema.columns WHERE table_schema='public' AND table_name='association_events' AND column_name='discovery_user_id') AND EXISTS(SELECT 1 FROM information_schema.columns WHERE table_schema='public' AND table_name='organization_invitations' AND column_name='target_user_id')",
      "schema: wiki75 and discovery77 columns"
    ),
  ].join("\n") + "\n";
const emptyTenancy =
  "u.seat IS NULL AND u.church_id IS NULL AND u.sending_church_id IS NULL AND u.sending_network_id IS NULL";
const ownProfile = (key, network) =>
  `EXISTS(SELECT 1 FROM users u JOIN discovery_profiles p ON p.user_id=u.id WHERE u.id=${u(key)} AND ${emptyTenancy} AND p.sending_church_id IS NULL AND p.sending_network_id ${network ? `= ${q(network)}::uuid` : "IS NULL"})`;
const answered = (invitationId, actor, status) =>
  `EXISTS(SELECT 1 FROM organization_invitations WHERE id=${q(invitationId)}::uuid AND status=${q(status)} AND responded_by=${actor} AND responded_at IS NOT NULL)`;
const eventCount = (key, event, org, actor) =>
  `(SELECT count(*) FROM association_events WHERE subject_type='discovery' AND discovery_user_id=${u(key)} AND org_id=${q(org)}::uuid AND event=${q(event)}${actor ? ` AND actor_user_id=${actor}` : ""})`;
const stepFiles = {
  "00-schema.sql": schemaOracle,
  "01-baseline.sql": baselineOracle,
};
manifest.oracles = { schema: "00-schema.sql", baseline: "01-baseline.sql" };
for (const [index, entry] of signup.entries()) {
  const sending = entry.type === "church_to_sending_church";
  const subject = `SELECT id FROM users WHERE email=${q(entry.email)}`;
  const org = sending ? orgs.sendingChurch.id : orgs.network.id;
  const type = sending ? "discovery_to_sending_church" : "discovery_to_network";
  const filename = `0${index + 2}-${entry.key}.sql`;
  manifest.oracles[entry.key] = filename;
  stepFiles[filename] =
    [
      assertion(
        `(SELECT count(*) FROM users u JOIN discovery_profiles p ON p.user_id=u.id WHERE u.email=${q(entry.email)} AND u.name=${q(entry.name)} AND ${emptyTenancy} AND ${sending ? `p.sending_church_id=${q(org)}::uuid AND p.sending_network_id IS NULL` : `p.sending_network_id=${q(org)}::uuid AND p.sending_church_id IS NULL`})=1`,
        `${entry.key}: seatless discovery association persisted`
      ),
      assertion(
        `${answered(entry.invitationId, `(${subject})`, "accepted")} AND EXISTS(SELECT 1 FROM organization_invitations WHERE id=${q(entry.invitationId)}::uuid AND type=${q(type)} AND target_user_id=(${subject}) AND target_church_id IS NULL AND target_sending_church_id IS NULL)`,
        `${entry.key}: intended invitation claimed and bound`
      ),
      assertion(
        `(SELECT count(*) FROM association_events WHERE source_invitation_id=${q(entry.invitationId)}::uuid AND subject_type='discovery' AND discovery_user_id=(${subject}) AND org_id=${q(org)}::uuid AND event='associated' AND actor_user_id=(${subject}))=1`,
        `${entry.key}: one actual acceptance audit`
      ),
      assertion(
        `NOT EXISTS(SELECT 1 FROM persons WHERE user_id IN (${subject}))`,
        `${entry.key}: no plant person provisioned`
      ),
      assertion(
        `EXISTS(SELECT 1 FROM churches WHERE id=${q(church.id)}::uuid AND name=${q(church.name)}) AND EXISTS(SELECT 1 FROM sending_churches WHERE id=${q(orgs.sendingChurch.id)}::uuid AND name=${q(orgs.sendingChurch.name)}) AND (SELECT count(*) FROM sending_networks WHERE (id=${q(orgs.network.id)}::uuid AND name=${q(orgs.network.name)}) OR (id=${q(orgs.foreignNetwork.id)}::uuid AND name=${q(orgs.foreignNetwork.name)}))=2`,
        `${entry.key}: run-owned plant and organization identities preserved`
      ),
    ].join("\n") + "\n";
}
const lifecycleSteps = {
  accept: [
    assertion(
      answered(invitations.acceptNetwork, u("lifecycle"), "accepted"),
      "accept: claimed intended invitation"
    ),
    assertion(
      ownProfile("lifecycle", orgs.network.id),
      "accept: independent slot without tenancy"
    ),
    assertion(
      `${eventCount("lifecycle", "associated", orgs.network.id, u("lifecycle"))}=1`,
      "accept: one audit"
    ),
  ],
  decline: [
    assertion(
      answered(invitations.declineSending, u("lifecycle"), "declined"),
      "decline: intended answer recorded"
    ),
    assertion(
      ownProfile("lifecycle", orgs.network.id),
      "decline: prior network unchanged, sending slot empty"
    ),
    assertion(
      `NOT EXISTS(SELECT 1 FROM association_events WHERE source_invitation_id=${q(invitations.declineSending)}::uuid)`,
      "decline: no association audit"
    ),
  ],
  leave: [
    assertion(
      ownProfile("lifecycle", null),
      "leave: profile retained, association cleared"
    ),
    assertion(
      `${eventCount("lifecycle", "disassociated", orgs.network.id, u("lifecycle"))}=1`,
      "leave: one account-attributed audit"
    ),
  ],
  sever: [
    assertion(
      ownProfile("sever", null),
      "sever: profile retained, association cleared"
    ),
    assertion(
      `${eventCount("sever", "disassociated", orgs.network.id, u("networkOwner"))}=1`,
      "sever: one Owner-attributed audit"
    ),
  ],
};
for (const [index, [name, checks]] of Object.entries(
  lifecycleSteps
).entries()) {
  const filename = `0${index + 5}-${name}.sql`;
  manifest.oracles[name] = filename;
  stepFiles[filename] = checks.join("\n") + "\n";
}
// These three phases assert only their own state; seat/transfer order is independent.
for (const [name, filename, checks] of [
  ["transfer", "09-transfer.sql", postSafeChecks.slice(1, 8)],
  ["associatedSeat", "10-associated-seat.sql", [postSafeChecks[8]]],
  ["emptySeat", "11-empty-seat.sql", postSafeChecks.slice(9, 11)],
]) {
  manifest.oracles[name] = filename;
  stepFiles[filename] = [...checks, ...wikiChecks].join("\n") + "\n";
}
// Fixture is independently held to the same migration identity before any INSERT.
sql.splice(4, 0, schemaOracle);
...
console.log(
  `Prepared 19 private offline artifacts for ${run}; no database, authentication or provider calls.`
);
