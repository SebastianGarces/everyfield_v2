...
Evry must adopt a compatible team-first locking protocol as well as the field adaptations above. A lock acquired inside the same SQL statement as predicate reads does not provide the fresh post-wait snapshot used here. Its current single-statement atomic effect must account for that distinction while preserving confirmation, staleness and retry behavior. Native-only success does not establish combined release readiness. The subsequent seat-removal proof below covers native assignment against AS-016. Other account-tenancy writers remain outside this concurrency claim. An assignment that loses to role deletion rolls back on the existing FK; its user-facing error mapping is unchanged.
...
The Evry proposal is `scripts/proofs/team-leader-provenance-830-evry-protocol.md`, with a local handoff copy at `/private/tmp/leadership830-evry-protocol.md`. The pinned Evry writer uses Serializable isolation, which retains a snapshot from before a blocking advisory-lock SELECT. The proposal therefore specifies a narrowly routed READ COMMITTED batch with shared plant locking, required dependent row locks, exact stale-plan checks and unchanged receipt replay semantics. It is a proposal for the Evry owner, not an implemented or validated Evry change.
