...
// ----------------------------------------------------------------------------
// The reference clock — the single source of truth for ALL relative dates.
//
// ANCHORED TO THE SEED RUN, NOT TO A LITERAL. It used to be a hard-coded
// `2026-06-22`, and the #538 fleet pass is what exposed the cost: assessments
// are generated by `generateAssessment`, which builds its snapshot at the REAL
// current time and takes no `asOf`. Two months after that literal was written,
// every plant in the corpus reported a stall clock of 63–100 days and a last
// vision meeting 67 days ago — so all twelve read as stalled and overdue,
// including the launch-ready exemplar whose whole job is to be the plant that
// must NOT escalate. The corpus had quietly stopped testing what it was built
// to test, and nothing said so.
//
// Determinism survives where it matters: every offset below is still derived,
// index-based and free of `Math.random`, so one run is reproducible and two
// runs on the same day are identical. What changes is that the corpus is always
// the same AGE rather than always the same DATE — and age is what every signal
// in the engine actually reads.
//
// Midnight UTC, not the wall clock, so a run at 09:00 and a run at 23:00 on the
// same day produce identical data.
// ----------------------------------------------------------------------------
