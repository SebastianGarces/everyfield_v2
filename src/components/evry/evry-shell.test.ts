...
const read = (file: string) =>
  readFileSync(`src/components/evry/${file}`, "utf8");
...
test("production shell uses Eve native transport, not the retired model/run endpoints", () => {
  ...
  assert.doesNotMatch(
    read("evry-shell.tsx"),
    /readEvryConversationStream|reconnectEvryRun|\/api\/evry\/(?:requests|runs|conversations)|attachments\/plan/
  );
});
