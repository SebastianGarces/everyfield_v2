...
export function PersonForm({
  ...
  return (
    <>
      {/* The photo is bytes with their own action, so its control sits OUTSIDE
          this form — a nested form is not legal markup, and the profile form
          may not carry a storage key (P-024a). Edit only: the key convention
          names a person, and a person being created does not have an id yet. */}
      {isEdit && person && (
        <div className="mb-6">
          <PersonPhotoField person={person} />
        </div>
      )}
