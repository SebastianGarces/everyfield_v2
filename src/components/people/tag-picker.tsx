...
export function TagPicker({
  ...
  // AS-020: assigning, creating and REMOVING a tag are all `people.write`. The
  // remove is the easy one to miss — it is the small × on each badge, not a
  // button in this file — so the badge is handed no `onRemove` at all for a
  // read-only viewer and renders as a plain label.
      ...
      <div className="mb-2 flex flex-wrap gap-1.5">
        {selectedTags.map((tag) => (
          <TagBadge
            key={tag.id}
            tag={tag}
            onRemove={canWrite ? () => handleSelectTag(tag) : undefined}
          />
        ))}
        {!canWrite && selectedTags.length === 0 && (
