...
import {
  dismissPhaseTemplatePromptAction,
  importPhaseTemplatesAction,
} from "@/app/(dashboard)/tasks/phase-prompt-actions";
import {
  ClearReceiptCookie,
...
// Answering
//
// THE TWO ACTIONS MOVED OUT (#498 review). They were inline `"use server"`
// closures here, which publishes two POST endpoints — one of them creating
// 22–26 tasks — that the export-walk enforcing the seat guard cannot see: it
// reads the exports of modules whose PROLOGUE carries the directive. They now
// live in `@/app/(dashboard)/tasks/phase-prompt-actions`, guarded
// `tasks.write` and `phase.signal`, and this file passes them to the island
// exactly as before.
// ----------------------------------------------------------------------------
