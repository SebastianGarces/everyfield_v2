...
/**
 * Delete a team and detach any meetings or reporting relationships that point at it.
 */
export async function deleteTeam(
  ...
  await db.transaction(async (tx) => {
    ...
    await tx
      .update(ministryTeams)
      .set({ reportsToTeamId: null, updatedAt: new Date() })
      .where(
        and(
          eq(ministryTeams.churchId, churchId),
          eq(ministryTeams.reportsToTeamId, teamId)
        )
      );
