...
// decides by matching substrings in an error whose SHAPE it assumes. An
// assumption written in a comment is not a guard: if the constraint name
// arrives somewhere this function does not look, it returns null, the raw error
// rethrows, and nobody finds out until a planter reads Postgres at them.
//
...
//     the driver's `NeonDbError` DIRECTLY, so the constraint name is in
//     `message` and `cause` is undefined (§1);
//   * a single-statement write is wrapped by Drizzle in
//     `Failed query: <sql>\nparams: <…>` with that `NeonDbError` hung on
//     `cause`, and the wrapper names no constraint at all (§2).
//
// So reading only `cause` would miss the reactivation, and reading only
// `message` would miss every single-statement path. Do not "simplify" this to
// one of them — that is precisely the regression this file exists to see.
// ----------------------------------------------------------------------------

const ROLE_INDEX = "team_memberships_role_active_unique_idx";
