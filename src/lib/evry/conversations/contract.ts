...
const evryCompletedStepSchema = z
  .object({
    ...
    resultCode: z.string().regex(SEMANTIC_KEY_PATTERN),
...
export const evryConversationStateDocumentSchema = z
  ...
  .superRefine((state, context) => {
    ...
    for (const [index, choice] of state.explicitChoices.entries()) {
      const selected = referenceByKey.get(choice.referenceKey);
      if (!selected || selected.entityId !== choice.selectedEntityId) {
        context.addIssue({
          code: "custom",
          path: ["explicitChoices", index],
          message: "Evry choices must select a persisted resolved reference",
        });
      }
    }
