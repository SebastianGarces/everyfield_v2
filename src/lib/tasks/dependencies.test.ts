...
const snapshot = JSON.parse(
  read("src/db/migrations/meta/0048_snapshot.json")
) as {
  tables: Record<
    string,
    {
...
test("task_dependencies unique-indexes the edge pair", () => {
  ...
  assert.ok(
    edge,
    "task_dependencies_edge_unique_idx missing from the 0048 snapshot"
  );
...
test("both task FKs are composite onto (id, church_id) and CASCADE", () => {
  ...
  assert.ok(table, "public.task_dependencies missing from the 0048 snapshot");
  ...
  for (const name of [
    ...
    assert.ok(fk, `${name} missing from the 0048 snapshot`);
...
test("tasks carries the (id, church_id) unique index the composite FKs need", () => {
  ...
  assert.ok(
    index,
    "tasks_id_church_id_unique_idx missing from the 0048 snapshot"
  );
...
test("a self-loop is a CHECK", () => {
  ...
  assert.ok(
    check,
    "task_dependencies_no_self_check missing from the 0048 snapshot"
  );
...
test("the migration builds the unique index on tasks BEFORE the composite FKs", () => {
  const migration = read("src/db/migrations/0048_task_dependencies.sql");
  assertInOrder(
    migration,
    "0048_task_dependencies.sql",
    [
