 ...
 * Member's sentence is a COPY DECISION rather than a translation.
 *
 ...
 *   * Describe the list and stop ("Your plant's tasks and follow-ups"). That is
 *     the shape /people and /teams take, and here it would UNDER-claim: it
 *     tells a Member with three assigned tasks that this is a page to look at,
 *     when the complete control on each of their own rows is right there. An
 *     over-hide in sentence form is the same defect pointed the other way, and
 *     the sweep is not allowed to commit it either (`read-only-surfaces.ts`).
 *
 * The sentence therefore names the list AND the one verb they hold. Its second
 * clause is an imperative on purpose: an instruction is honest exactly when its
 * reader may perform it, which is the same reason /meetings/new keeps "Set a
 * date, time, and location…" — the rule was never "no imperatives", it is "no
 * imperative for a write this reader would be refused".
 *
 ...
 */
export const TASKS_SUBTITLE_FOR_A_READER =
