...
import type { InvitationCreateInput } from "@/lib/validations/meetings";
import { and, desc, eq, sql } from "drizzle-orm";
import type { InvitationLeaderboardEntry, InvitationSummary } from "./types";

/**
 * Create an invitation record.
 * Requires an inviteeId (person record) — inviteeName is auto-populated.
 */
export async function createInvitation(
  ...
  // Look up the person to auto-populate inviteeName
  const [person] = await db
    .select({ firstName: persons.firstName, lastName: persons.lastName })
    .from(persons)
    .where(eq(persons.id, data.inviteeId))
    .limit(1);

  const inviteeName = person
