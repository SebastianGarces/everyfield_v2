...
export default async function SeatInvitationPage({
  searchParams,
}: {
  ...
  const [described, { user }] = await Promise.all([
    describeUserInvitationForRegistration(token),
    getCurrentSession(),
  ]);
  if (!described || described.invitedAs.kind !== "seat")
    return (
      <Card className="w-full max-w-md">
        <CardHeader>
          ...
          <CardDescription>
            ...
            the team that invited you to send a new one.
          </CardDescription>
        </CardHeader>
      </Card>
    );
    ...
    <Card className="w-full max-w-md">
      ...
      <CardContent className="space-y-4">
        <p className="text-muted-foreground text-sm">
          This invitation only works for{" "}
          <strong className="text-foreground">{described.inviteeEmail}</strong>.
        </p>
        {user ? (
          <AcceptSeatInvitation token={token} />
        ) : (
          <div className="space-y-3">
            ...
            <p className="text-muted-foreground text-sm">
              ...
              <Link
                ...
                href={loginPathFor(seatInvitationPath(token))}
              >
                Sign in to accept
              </Link>
              .
