...
function routeEntries(repoRoot: string): TeamsEvrySurface[] {
  return collectRouteSurfaces(repoRoot)
    ...
    .map((surface): TeamsEvrySurface => {
      // Meetings owns the shared route; Teams still owns its scoped service read.
      if (surface.path === "/teams/[teamId]/meetings") {
        return {
          kind: "route",
          identity: surface.identity,
          ...
          mutationShape: null,
          classification: { state: "excluded", reason: "owned_by_meetings" },
        };
      }
      const identity =
