...
- The line above is UNIVERSAL and is pinned universally (round 8, 2026-08-10): §1b‴ WALKS every `"use server"` module under `src/`, matches braces to read whole bodies, and derives what counts as a mint per module (`verifySession`/`getCurrentSession`, plus any local or imported helper that reaches one — `currentViewer`, `requirePlatformAdmin`). Twice the rule was written down as universal while a file list made it domain-wide, and both times the unlisted modules were the ones parsing first — five of them at round 7. The only exemption is the `(auth)` and `(marketing)` route groups, whose endpoints are public by construction, and the exempt set is asserted EXACTLY (`login`, `register`, `requestInviteAction`) so the group cannot become a hiding place. Above the `try`, not merely first inside it: a sessionless call must THROW, never be converted into a handled `{ success: false }` by a catch.
