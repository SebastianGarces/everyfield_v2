...
- ⚖ A RE-STAMP ADDS ONE SECOND, NEVER A DAY (ruled 2026-08-21, #566). A rebase that puts the newest migration's `when` at or below a sibling is repaired by `pnpm exec tsx scripts/restamp-migration.ts` — the tail moves to `max(its own, highest sibling + 1000ms)`, in the journal AND in its own `-- DELETE FROM … WHERE created_at = …` rollback header, which are the only two places a `when` lives. Four hand re-stamps of +24h put the journal ~46h ahead of the wall clock, so the next honest `Date.now()` landed BELOW the tail; one second lets real time close the gap instead. `pnpm db:generate` runs the script, so a collision resolves itself. Entries behind the tail are history and are never re-stamped.
