...
- **Never interpolate a slug into a wiki path** (`` `/wiki/${slug}` ``). Build every wiki path — `href`/`router.push`, OpenGraph `url`, `revalidatePath`, and the `pathname === wikiHref(slug)` active-item test — with `wikiHref()` from `src/lib/wiki/href.ts`. It encodes **per segment**, so `/` stays a separator for the `[...slug]` catch-all and safe slugs are byte-identical.
...
- The rule nonetheless covers every call site, because only some are ever parsed as a URL: `<a href>`/`<Link>`, `router.push`, and `openGraph.url` (Next resolves it with `new URL(path, metadataBase)`). `revalidatePath` is matched as a **literal** cache key against the request's already-encoded path, and the sidebar's `pathname === wikiHref(slug)` is a string compare against `usePathname()`, likewise already encoded — there a raw space is a silent mismatch: no revalidation, no highlighted item, no error. Encoding is a no-op for well-formed slugs, so routing every site through `wikiHref` costs nothing and removes the judgement call at the point of use. Pinned by `src/lib/wiki/href.test.ts`, which measures each character against `new URL` directly.
