...
- …so a migration rebased behind a sibling is RE-STAMPED to `previous.when + 1000`, ONE SECOND, never plus a day. `seat.test.ts` → "the journal never regresses" catches the bad stamp in the gating CI job, so this never reaches a database — but the repair compounds: `drizzle-kit generate` stamps a real `Date.now()`, so every day of head-room a track grants itself is a day the NEXT author's honestly-generated stamp lands below the tail and fails for a reason that has nothing to do with their change. 0055–0057 walked the journal two days past the wall clock exactly this way (#190).
