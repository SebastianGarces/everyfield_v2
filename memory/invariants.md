...
- No invitation that cannot be answered: every TYPE that can name an existing account needs an in-app surface for the role that answers it. Plant-side (`church_to_*`) is `/settings/association` + the dashboard reminder; `sending_church_to_network` targets a sending-church admin and its surface is #304 WS3 — until that lands, the type is targetable with nowhere to answer.
