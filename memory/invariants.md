...
- REMOVING A SEAT CLEARS THE TENANCY AND THE SEAT AND NOTHING ELSE ABOUT THE PERSON (AS-016). The account ROW survives with both NULL, and the `persons` row and its `team_memberships` are untouched — a person record and an account are separate things, so losing the login must never lose the roster entry. The cascade's other three effects are sessions deleted, OPEN tasks (`status <> 'complete'`) reassigned to the Owner, and `ministry_teams.leader_id` nulled for the teams that account's own person record led.
- The removal is ONE `db.batch` with the TENANCY CLEAR LAST. The three earlier statements are redo-safe, so a replayed request re-runs three no-ops; the marker's `WHERE` re-asserts the actor's plant and `seat <> 'owner'`, so a stale pre-read commits nothing. Moving the marker up trades redo safety away silently — `src/lib/seats/roster.test.ts` fails on the reorder.
