...
- **An invitation may name no target, and that is not a hole — it is the register path (#23).** `resolveInvitationRequest` accepts a request with neither target set: the invitee had no account when the admin typed their address, so there is no row to point at. Such a row can never be ACCEPTED as it stands — `verifyInvitationAuthority` compares the actor's own org id against a `null` target and refuses, and `lockTargetRow`/`unboundTargetSlot`/`associationStatement` each throw on a missing target — so the only way it becomes an association is `bindOpenInvitationTarget` (`src/lib/invitations/core.ts`), a compare-and-set on `pending` + BOTH targets null + unexpired, which is what makes an invite link SINGLE USE. Registration runs it BEFORE `acceptInvitationAs`, never after: binding leaves the row `pending` with a target (recoverable — the planter can answer later), whereas claiming first and creating the organization second would, on a crash, leave an invitation reading `accepted` with no association behind it. `createAccountEntities` therefore sets NEITHER oversight FK; the association is only ever written by the accept batch, gated on the claim.
- **No invitation that cannot be answered — so an address that ALREADY has an account is refused at create** (RULED 2026-08-04, #23). `resolveInvitationTarget` (`src/lib/invitations/core.ts`) looks the address up in `users` and returns `ACCOUNT_EXISTS_MESSAGE` for ANY account, whatever its role and whether or not it has an org: the only place an invitation can be answered today is `/register` (the link creates the org and redeems in one request), and somebody who already registered cannot register again — the planter-side association area is #277. So `createInvitation` currently issues only OPEN invitations, and `assertTargetSlotFree`/`heldOversightSlot` are reachable only for a hand-built targeted row until #277 restores the targeted path (role → target FK mapping; see the doc comment on `ACCOUNT_EXISTS_MESSAGE`). Same family as the occupied-slot refusal below. The check is in the logic layer, so a forged direct call is refused too; one message for every account, deliberately, since the finer-grained refusals it replaced told an inviter what KIND of account sits behind an address. Pinned by `invitations-ui.test.ts` §6 (`inviteeAccountRefusal` is pure and executed for every role)
- **An invitation token is bound to the invited ADDRESS, not to whoever holds the link** (RULED 2026-08-04, #23). An invite link is a uuid in a URL — forwarded, pasted, archived — and it buys two things: the association, and (when `BETA_INVITE_CODE` is set) a bypass of the beta gate. Both are now conditioned on the registering email equalling `organization_invitations.invitee_email`, normalized trim+lowercase: `registrationEmailMatchesInvitation` in `(auth)/register/beta-gate.ts`, called from `register` (before the gate, before any account exists) and from inside `hasValidInvitationBypass(invitationId, email)`. A mismatch is a field error naming the invited address; a wrong address is fixed by REVOKING and re-inviting, never by re-aiming a live invitation. A row with NO recorded address (pre-#23) matches nobody — `describeInvitationForRegistration` returns `null` for it, so it no longer registers anyone. The register form pre-fills the address and marks it `readOnly` (not `disabled` — a disabled input submits nothing), but that is convenience: the action is a POST that never saw the form
- **Whether the target's slot is free is now asked TWICE, and the two are not interchangeable.** `createInvitationAs` refuses up front when the plant already belongs to an oversight org (ruled 2026-08-03, #23 — `SLOT_TAKEN_MESSAGE` / `ALREADY_OURS_MESSAGE`, distinct messages because the next action differs). That is a SELECT-then-INSERT check: two admins racing still both get a row, and it is deliberately NOT the guard. The guard is the accept-time one above (`unboundTargetSlot` + `lockTargetRow`), unchanged. Deleting the create-time check loses only the early, legible refusal; deleting the accept-time one loses correctness.
- **An invitation belongs to the inviting ORG, not to the admin who typed it** (RULED 2026-08-04, #23). The pending list is org-scoped, so revoke is too: `revokeInvitationQuery` matches on `sending_church_id`/`sending_network_id` from the session and no longer on `inviter_user_id`, and the surface dropped its per-row `canRevoke`. The list and the revoke are built from ONE predicate, `invitingOrgOf(actor)` (`core.ts`), so they cannot disagree about what "ours" means — `service.test.ts` reads both WHERE clauses and compares them. Strictness is unchanged: any role that does not invite, and any oversight admin with no org of their own, produces `false` and matches nothing
...
- **A shared secret is never compared with `===`.** Both cron routes call `matchesBearerSecret` from `src/lib/security/constant-time.ts` — `===` short-circuits on the first differing byte (a timing oracle), and a plain `if (a.length !== b.length)` guard in front of `timingSafeEqual` only relocates the leak onto the secret's length, so both sides are SHA-256'd to a fixed 32 bytes first. One `CRON_SECRET` authorises BOTH `/api/notifications/dispatch` and `/api/phase-engine/assess` (`contracts/config.md`), which is why the comparison is shared rather than copied: hardening one route while the other keeps `===` leaks the key to both (#266, ruled 2026-08-04). Enforced by `src/lib/security/constant-time.test.ts`, which scans every route reading `CRON_SECRET` or `REVALIDATION_SECRET`. `REVALIDATION_SECRET` joined the table with #310 — `src/app/api/wiki/revalidate/route.ts` now compares it with `constantTimeEquals` in a single shared guard that both POST and DELETE call.
- **A request header the app itself does not write is client input, so nothing may branch on one.** Nothing wrote `x-is-crawler`: `src/proxy.ts` set it on the RESPONSE (`NextResponse.next()`), so to the dashboard layout reading it off the REQUEST a proxy-set value and a forged one were *indistinguishable* — which is precisely why branching on it was unsafe (#240, removed; `crawler.test.ts` greps the whole tree for the name). The trustworthy proxy→app channel is `NextResponse.next({ request: { headers } })`, and only when the proxy writes it UNCONDITIONALLY on every continuation so a client value is always overwritten. `PATHNAME_HEADER` (`x-pathname`), stamped in `proceed()` in `src/proxy.ts`, is the one such header; its absence means the proxy did not run, and every reader must fail closed on that.
- **The crawler allowance is ONE predicate over two inputs, on a fixed list of routes.** `isCrawlerPreviewRequest(user-agent, pathname)` in `src/lib/crawler.ts` — called by the proxy with `request.nextUrl.pathname`, by the `(dashboard)` layout with the stamped `x-pathname` — is true only for a known crawler UA on `CRAWLER_PREVIEWABLE_ROUTE_PREFIXES` (`/dashboard`, `/wiki`, `/oversight`): the routes whose pages render without a session (ruled 2026-08-04). That list is spread into the proxy's `PROTECTED_ROUTE_PREFIXES`, so "previewable ⊆ protected" holds structurally. Every other route in the group bounces a crawler to /login like anyone else — handing the session-less shell to a page that needs a session turns a clean 307 into a 500 for anything carrying a crawler token, WhatsApp's in-app browser (a human) included. The allowance is a User-Agent allowance and always was: a forged UA gets the unauthenticated shell around pages that already render session-less (wiki articles, for their OG tags), never a session and never per-user data, which stay behind `getCurrentSession()` in the page.

**Source:** `src/lib/auth/session.ts`, `src/lib/auth/cookies.ts`, `src/lib/invitations/service.test.ts`, `src/lib/security/constant-time.ts`, `src/lib/crawler.ts`
...
- **Never interpolate a slug into a wiki path** (`` `/wiki/${slug}` ``). Build every wiki path — `href`/`router.push`, OpenGraph `url`, and the `pathname === wikiHref(slug)` active-item test — with `wikiHref()` from `src/lib/wiki/href.ts`. It encodes **per segment**, so `/` stays a separator for the `[...slug]` catch-all and safe slugs are byte-identical.
- **`revalidatePath()` is the one wiki path `wikiHref` must NOT build** — use `wikiRevalidationPath()` from `src/lib/wiki/service.ts` (#310). `revalidatePath` uses its argument verbatim as a cache tag, and the tag a rendered page carries is derived from the **decoded** pathname with only `/ # ? %2f %23 %3f %5c` re-escaped — not from the encoded href. The two forms coincide for every URL-safe slug, which is why the wrong one survived review; for a slug with a space, `#`, `?`, `%` or a non-ASCII character the href form matched no tag and revalidated **nothing**, while still returning 200. `service.test.ts` pins the form against Next's own `decodePathParams` and scans the source so `revalidatePath(wikiHref(` cannot come back.
- Why, precisely — the earlier "a space … breaks the link outright" was wrong. Slugs are authored content, and in the WHATWG URL parser's path state `#` **ends the path** and starts the fragment while `?` ends it and starts the query (both yield a valid URL aimed at an article that does not exist), and `%` passes through **verbatim**, so the damage lands when Next percent-decodes the route param (`100%` throws `URIError`; `50%20off` silently decodes to `50 off`). A **space is in the path percent-encode set, so the parser escapes it for us** — the one character a *parsed* call site survives raw.
- The rule nonetheless covers every call site, because only some are ever parsed as a URL: `<a href>`/`<Link>`, `router.push`, and `openGraph.url` (Next resolves it with `new URL(path, metadataBase)`). The sidebar's `pathname === wikiHref(slug)` is instead a **literal** string compare against `usePathname()`, which is already encoded — there a raw space is a silent mismatch: no highlighted item, no error. Encoding is a no-op for well-formed slugs, so routing every site through `wikiHref` costs nothing and removes the judgement call at the point of use. Pinned by `src/lib/wiki/href.test.ts`, which measures each character against `new URL` directly.
