...
- ⚖ …BUT IT STILL RECONCILES, AFTER THE PUSH (#527): `void action(id).then(() => router.refresh()).catch(() => {})`. Chained on the action's promise, the refresh runs once the push is issued and re-renders where the user now IS. Skipping it is a bug, and #308 shipped it skipped on a FALSE premise — "the destination's own render is a fresher read". A client-side push REUSES the layout segments both routes share rather than re-rendering them (partial rendering; `.next-docs/01-app/02-guides/authentication.mdx:1350`), and the shell's bell sits in exactly such a shared layout, so nothing re-read it on arrival: 26 unread, row click, `read_at` set, bell stuck at 26 for the session. Re-measured on #527's preview: 22/22 navigated AND the count fell on the destination. The `.catch` is part of the shape — an unhandled rejection on a page the user has left has nobody to tell.
