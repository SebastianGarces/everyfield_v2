...
- A church holds at most ONE fork of any system template: `message_templates_church_fork_unique_idx` (partial, `(church_id, source_template_id) where source_template_id is not null`) plus `forkTemplate`'s `ON CONFLICT … DO NOTHING`, re-reading the winner's fork on an empty `returning()` — the loser ADOPTS the winner, because a fork holds the planter's own edits. The NULL gap (nullable `church_id`) is deliberate: nothing writes that shape, and a `coalesce` expression index would make the ON CONFLICT target unspellable.
- A (meeting, person) pair holds at most ONE unanswered RSVP: `meeting_confirm_tokens_pending_unique_idx` (partial, `(meeting_id, person_id) where status = 'pending'`) plus ONE `ON CONFLICT … DO UPDATE` in `createConfirmationToken`. An EXPIRED pending row is renewed IN PLACE, deliberately invalidating its old emailed link; a LIVE token is never rotated — the call writes nothing and the live token is re-read.
- One person per team role: `team_memberships_role_active_unique_idx` (partial, `role_id` ALONE where `status = 'active'`) — `(church_id, role_id)` was rejected because a forged church id would buy a second seat. The INSERT refuses with an empty `returning()` (`ON CONFLICT … DO NOTHING`); `isSeatConflict` (`src/lib/ministry-teams/membership-conflict.ts`, via `isUniqueViolation` — the ONE shared copy of that predicate) is all that stands between a lost race and a raw driver error reaching a planter.
- That is the ONLY unique index on `team_memberships` — 0039 dropped the subsumed triple `team_memberships_active_unique`, because ANY second unique index is a non-arbiter a raced INSERT meets first and raises 23505 past the `DO NOTHING`. Never re-add one, never widen the `ON CONFLICT` target, never add a pre-flight SELECT; `ruled-guards.test.ts` §4c/§4d pin the one recognised name and, off the RENDERED table, the exactly-one-index shape.
- The REACTIVATION path is an UPDATE and the index is NOT its guard for a same-person double submit (one row cannot collide with itself): it carries `status = 'inactive'` in its own WHERE — a compare-and-set, not the banned pre-flight SELECT — and its empty `returning()` refuses exactly as the INSERT's does (`ruled-guards.test.ts` §3c).
...
- A migration that adds an arbiter index goes FIRST — the deploy order is NOT free: `ON CONFLICT (…) WHERE …` against a database lacking the index is SQLSTATE 42P10 on EVERY call. Nothing in this repo applies migrations on deploy, so code-first is the default unless an operator runs `pnpm db:migrate`.
...
- A race guard is only proven against a REAL Postgres: the `LIVE_DB_TESTS=1` suites reach one through a local Neon HTTP proxy, switched by the test-runner PRELOAD `scripts/live-db-endpoint.ts` — never a second driver (`node-postgres` has no `db.batch`, so it exercises code the app never runs) and never a seam in `src/db/index.ts`, which stays two lines and interprets no hostname.
- A live suite MINTS every row it needs, actor included, inside its own `SCRATCH_NAME` namespace — never `select … from users limit 1`. `src/db/live-suite-coverage.test.ts` fails if a suite opts into `LIVE_DB_TESTS` without joining `test:live`, if the preload is dropped, or if any module under `src/` touches `neonConfig`.
...
- ⚖ PUSH is far narrower: an oversight recipient gets ONLY the daily digest and three milestone events, and `enqueue` refuses every granular category for them, gated by `share_activity_with_oversight`.
...
- A NEW action mints ABOVE the `try`, so a sessionless call THROWS instead of becoming a handled `{ success: false }`. NAMED RESIDUAL: 45 older `(dashboard)` exports mint inside their `try`, listed in `TRY_WRAPPED_MINTS` and retired when it empties.
...
- The crawler allowance is ONE predicate, `isCrawlerPreviewRequest(userAgent, pathname)`, over a route list that is a STRICT subset of the protected list — `/wiki` only. It buys the shell, never a session.
...
- `OVERSIGHT_ROLES` and `CHURCH_LEVEL_ROLES` are declared ONCE, in the IMPORT-FREE leaf `src/lib/auth/roles.ts`, because `@/lib/auth/access` opens with `@/db`. EVERY site imports that leaf DIRECTLY; `access.ts` MUST NOT RE-EXPORT them.
...
- A wiki endpoint with NO caller is DELETED, not kept — predicating dead code doubles the tenancy surface. `service.ts` holds `wikiRevalidationPath` + `revalidateArticle` and NOTHING else; `service.test.ts` pins the PROPERTY (no `@/db` specifier, no `db.<verb>(` statement, exactly those two exports), and `write-paths.test.ts` fails on any export no non-test, non-barrel file in `src/` names.
- A guard's own COMPLETENESS is DERIVED, never asserted in prose: `readerFacingReads()` is `deepEqual`'d against every `*Query` export of `get-articles.ts` + `search.ts`, a second test pins that `*Query` builders are DECLARED only there and in `write-queries.ts`, and BOTH spellings of a declaration are read (`export function` AND `export const` — `valueExportStatements`, the shared reader for what a function-only walk cannot see).
...
- `tasks.parent_task_id` is a self-FK `ON DELETE CASCADE` (migration 0038), so a parent id naming no task is UNREPRESENTABLE — before the key, a forged parent made invisible work: hidden by `topLevelTasksOnly()`, still counted by `getTaskCounts`. CASCADE, never `set null` — an orphan promoted to a top-level task is a task the planter never created; soft deletes are unaffected.
...
- ⚖ The one stored thing is the ANSWER, a ROW in `phase_prompt_answers` unique on `transition_id`, which re-arms the prompt by itself — AND THE ROW IS ITS ONLY READER: `PHASE_TEMPLATE_PROMPT_COOKIE` is DELETED (#411), with `fastPathTransitionId` and the decline `status` that fed its mint. It saved no query (the answer arrives on the transition's own LEFT JOIN) and was a browser-held copy of an answer the PLANT owns; nothing browser-held may answer a prompt again.
...
- A `?status=` / `?priority=` / `?category=` on `/tasks` is PARSED, never cast — `parseTaskListSearchParams` (`src/lib/tasks/list-params.ts`), through the same zod enums the write schemas use, DROPPING what it does not recognise: a filter is a view, so unknown means unfiltered — the cast let a shared-link typo meet the column CHECK on a route with no error boundary.
...
- A still-live predicate (N-014) is ARMED BY A CALL FROM THE DISPATCHER'S OWN ENTRYPOINT — `registerNotificationConsumers()` (`notifications/register-consumers.ts`), at module scope in `/api/notifications/dispatch/route.ts` — never by a load-time side effect in the feature. `resolveLiveness` calls an unregistered type LIVE, and that route imports no feature module, so the load-time form (#321) armed every runtime EXCEPT the one that reads a predicate: six types MISSING in cron while both feature suites passed, each registering by hand first. A new consumer joins that one function; `route.test.ts` asserts the types over the ROUTE's graph and fails on a module-scope registration.
- ONE sync skeleton, `runNotificationSync` (`notifications/sync.ts`): cancel → plan → tally → swallow, plus `cancelEntityNotifications`; the 255-char `clampNotificationTitle` is `enqueueNotificationSchema`'s, in `enqueue.ts`. A feature owns only its facts, planner, differ, deps and predicate. Accepted residual: the older FAN-OUTs (`oversight.ts`, `plant-association.ts`) keep a tally loop each — they never cancel, so the shape does not fit; `sync.test.ts` names both and fails on a THIRD.
- The person↔user bridge has ONE spelling — `personIsUserInChurch` / `personHoldsLoginFilter` (`people/person-user.ts`). TENANCY, not formatting: a copy that drops `users.church_id` mails one plant's meeting to another plant's planter, and each consumer's own `.toSQL()` test still passes. `person-user.test.ts` fails on a second spelling.
- A meeting's reminder audience is RE-READ on every sync, never frozen at create — a vision meeting's guest list starts EMPTY — so `addToGuestList`/`removeFromGuestList` call `syncMeetingNotifications` with `previous` OMITTED: the meeting did not change, so no differ would cancel, and the cancel is what must run. `cancelByEntity` is entity-wide, so re-enqueuing the audience without someone is the only way their reminders stop.
- EVERY module that inserts into `tasks` asks for its notifications, `import.ts` and `events.ts` included — a template checklist and a meeting's follow-ups are the tasks a planter did not type and will forget. A directory property in `tasks/notifications.test.ts`, never a list of call sites.
...
- ⚖ That sentence names no window and carries no number. It lives in `EVALUATION_COMPARISON_EMPTY_COPY` (`src/lib/meetings/copy.ts`) so a guard can pin it by import.
...
- ⚖ The meeting-detail follow-up card is titled "Evaluation task", never "Follow-up completion": its query admits only `related_type = 'meeting'`. Widening it in the QUERY stays forbidden, because joining through attendance double-counts.
- NO `"use client"` MODULE REACHES `src/lib/meetings/service.ts`, directly or transitively, because it opens with `@/db` and the schema barrel — and it holds QUERIES ONLY. Anything a client component needs lives in one of SIX db-free siblings — `copy.ts`, `agenda.ts`, `meeting-type-filter.ts`, `evaluation-comparison.ts`, `labels.ts`, `evaluation-factors.ts`.
...
- ONE meeting-type filter table is rendered by BOTH `/meetings` and `/meetings/[id]/analytics`, and the module is named for what it HOLDS. The two DEFAULTS stay different on purpose: `"all"` for browsing, `"vision_meeting"` for analytics.
- Consolidating the two tables changed the ANALYTICS chip ORDER to the shared broadest-first list: cosmetic, declared rather than discovered.
...
- ONE MEETING DISPLAY VOCABULARY, `src/lib/meetings/labels.ts`: the type labels, both badge tints, the status labels, the create-form options AND the display title. It replaced seven hand-written tables plus four title derivations, already drifted where a planter could see it.
...
- `meetingDisplayTitle` is the title derivation for SEVEN ENUMERATED SURFACES (never "everywhere"), and the branch order IS the rule: a NUMBERED vision meeting outranks a stored title, a team meeting with a team falls back to `<Team> Meeting`, else `title || <type label>`. `labels.test.ts` walks the exact list and fails if a surface stops calling it or grows a local `title ??` back.
- CALLING the function is not SHARING the name: every field it branches on is REQUIRED and nullable (`MeetingTitleFacts`), never optional — an optional field lets a surface skip a SELECT and silently take a different branch; the projections test passes the real row shapes through the real function.
...
- `meetingTypeLabel` and `parseMeetingType` are the module's only string-keyed accessors, and each reads its table through `Object.hasOwn`, never a bare index — `"constructor"` reached `Object.prototype`, a defined value the `??` fallback never caught.
...
- The meeting-detail progress bar names itself by `aria-labelledby` pointing at the visible progress line. That line is a VALUE in `copy.ts`, which owns the one `task`/`tasks` grammar branch.
...
- The CALENDAR-DAY primitives live in `datetime.ts` and nowhere else: `MS_PER_DAY`, `toCalendarDate(date)`, `addCalendarDays(from, days)` — every `date` column written by tasks, ministry-teams and launch goes through them, so the day a write NAMES is measured in the zone it is later READ in. The rule is not yet absolute: a named 13-site debt list lives in [dates-times](invariants/dates-times.md) — route a call site whenever you touch its module; a NEW local day constant or a re-export through another module is the mistake this line stops.
...
- A message that a `router.refresh()` accompanies must NOT live inside the subtree that refresh re-renders — the refusal that fires the refresh unmounts its own `<Alert>` mid-read. Raise it through the root `<Toaster>`, a sibling nothing below it can unmount, and never delay the refresh for it.
