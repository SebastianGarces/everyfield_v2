...
- ⚖ A `users` row an oversight FK reached that holds an OVERSIGHT ROLE ADMINISTERING A DIFFERENT KIND OF ORG is a DATA DEFECT, never a recipient: `fanOutTo` counts it in `OversightFanOutReport.misprovisioned`, logs the role and the FK, and enqueues nothing. The exclusion moved from the `WHERE` into TypeScript so it can be COUNTED; `oversightReachCondition` is that defect probe and is never an audience.
- …and a row holding NO oversight role is ABSENT, not counted: that FK is an ordinary membership column, so a `team_member` carrying `sending_church_id` is a sending church's own staff. Counting them would fire the signal on the commonest row in the table.
...
- ⚖ A stored row on `UNSUBSCRIBE_CHANNEL` is a CHOICE, never inheritable: `preferenceValueIsInheritable` exempts that channel from #237's value-equality rule, so the emailed undo's "keep sending these" survives a flip of the coded default. The exemption is by CHANNEL, because nothing stored says who wrote a row.
