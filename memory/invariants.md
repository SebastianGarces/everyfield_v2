...
- The create-invitation success notice shows ONE neutral message for both kinds of invitation and never asserts whether an account exists; this surface renders no `/register?invitation=` link, and the action's success payload carries no target-derived field. A branching success answers the same question the collapsed refusal hides, for free.
