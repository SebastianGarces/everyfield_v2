...
- ⚖ That cost is paid ONCE in CSS, never by capping the token's lightness (ruled 2026-08-10, PR #387): `p a[href]` carries a permanent underline in the base layer of `src/app/globals.css`. Never delete that rule and never put `no-underline` on a link inside prose — either one puts SC 1.4.1 back at the mercy of wherever the token's L happens to sit.
