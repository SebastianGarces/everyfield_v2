...
- ⚖ AN OVERSIGHT ACCOUNT'S FEED IS SCOPED TO ITS ORG, NOT TO A CHURCH (N-027, #308): `notificationViewer` asks the tenancy FIRST, and `oversightScopedWhere` is ONE recipient predicate ANDed over THREE anchor arms — plants that are in the org's portfolio AND have `share_activity_with_oversight` on (a SUBQUERY, evaluated at read time, never an id list a caller assembled), the consent-EXEMPT types read off `OVERSIGHT_SHARING_EXEMPT_TYPES` itself, and `anchor_org_id`. That retires the write-only residual on org-anchored rows. Every user-facing path goes through `feedScopedWhere` — the list, the badge, the cold-start probe, the by-id read AND both mark-read UPDATEs — so a plant that has not opted in is unreachable by direct id and unmarkable, not merely unlisted.
...
- Use `useOptimistic` for instant feedback; the server action calls `refresh()` from `next/cache` to reconcile, not the client calling `router.refresh()`. Example: `ActivityTimelineClient`. Props-only is the other shape (`TagPicker`). ⚖ ONE EXCEPTION, and it is about WHO STAYS: when the same action is also reached by a click that NAVIGATES, the refresh moves to the client, because only the caller knows whether it is staying (`notification-feed.tsx`, #228).
- ⚖ A CLICK THAT NAVIGATES OWNS NEITHER A TRANSITION NOR A REFRESH (#228, #308). A `<Link>` whose `onClick` also fires a server action must call it plainly — not in `startTransition`, and not through an action that calls `refresh()`. Either one turns the click into work React owns on the route being LEFT: a transition entangles the push behind an update suspended on a round-trip, and the refresh re-renders the route the push is replacing. Measured on #308's preview, both strand the navigation (22/22 and 11/11) while the write still lands, which is the "it marked read and went nowhere" bug; with neither, 22/22 navigate. There is no optimistic state to keep on a page the user has left, and the destination's own render is a fresher read than a refresh of the screen being left.
