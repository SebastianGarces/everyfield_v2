...
- A coach cannot write anywhere, and it is STRUCTURAL rather than a rule to remember: every write verb is `tenancy: "plant"`, which demands a non-null `users.church_id`, and a coach the INVITATION FLOW created has none. Accepted residual: a SEEDED coach carries one (see Dev Seeds below), so it opens `/people` where an invited coach is bounced — fixtures are WIDER than the flow produces, and a check run against the seed is not a check of the flow. `/coaching/[churchId]` is the only `(dashboard)` route that names a church in its path, and the id in it is not authority — `readCoachedPlant` answers null for any plant the caller does not actively coach.
