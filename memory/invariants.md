...
- No invitation that cannot be answered, and it is now TRUE FOR ALL ROLES: every TYPE that can name an existing account has an in-app surface for the role that answers it. `church_to_sending_church` / `church_to_network` → the planter, at `/settings/association` plus the dashboard reminder; `sending_church_to_network` → the target sending church's admin, at the same route's second view (#304 WS3). `/settings/association` serves both roles and `/settings` links it to both; adding a fourth invitation type means adding its answering view in the same change (`answer-surfaces.test.ts` enumerates the type list and fails otherwise).
...
- A `sending_church_to_network` accept writes NO `association_events` row and sends NO milestone: both tables are church-scoped by a NOT NULL `church_id`, and its subject is a sending church. The invitation row (`status`, `responded_by`, `responded_at`) is its whole record, and the network reads the answer in its own invitations list. Auditing or notifying it needs the subject column + ruling `association-event.ts`'s header asks for — not a nullable tenant column.
...
- BOTH SIDES can sever (#304): the planter with `leaveOversightOrgAs`, the org's admin with `removePlantFromOrgAs`. Neither calls the three bare `disassociate*` primitives, which stay out of every `"use server"` module — a sever's FK write must assert the org it is severing (`fk = <this org>`) or it severs the wrong one for a plant that belongs to two.
