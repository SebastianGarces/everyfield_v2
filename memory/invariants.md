...
- Making that anchor per-church keeps the church out of the digest dedupe key STRING but makes its VALUE church-dependent: `currentDigestDedupeKeys` takes the church's anchor, never computing one current key set across churches. The sweep therefore walks ANCHOR COHORTS — one keyset scan per distinct (zone, weekday, hour) — which is what keeps its owed test an `IN (two literals)` (#448).
