...
- NO `"use client"` MODULE REACHES `src/lib/meetings/service.ts`, directly or transitively, because it opens with `@/db` and the schema barrel. Anything a client component needs lives in one of FOUR db-free siblings — `copy.ts`, `agenda.ts`, `meeting-type-filter.ts`, `evaluation-comparison.ts`.
