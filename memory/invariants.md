...
- **A request header the app itself does not write is client input, so nothing may branch on one.** `src/proxy.ts` sets headers on the RESPONSE (`NextResponse.next()`), which never reaches a Server Component — forwarding to the app would take `NextResponse.next({ request: { headers } })`, which the proxy does not do. So the dashboard layout's `headers().get("x-is-crawler")` could only ever have been satisfied by a client sending it (#240, unexploitable, removed). Where the proxy and a layout must agree, they derive the answer from the SAME request input via one shared predicate: the crawler allowance is `isCrawlerUserAgent(user-agent)` in `src/lib/crawler.ts`, called by both, and `x-is-crawler` no longer exists on either side (`crawler.test.ts` greps the tree for it). That allowance is a User-Agent allowance and always was — a forged UA gets the unauthenticated shell around pages that already render session-less (wiki articles, for their OG tags), never a session and never per-user data, which stay behind `getCurrentSession()` in the page.
