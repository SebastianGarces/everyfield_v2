...
- `association_events` names a SUBJECT, not a plant (ruled #351, migration 0033): `subject_type` ∈ {church, sending_church}, one nullable FK per kind, and a CHECK that exactly one is set. `church_id` is nullable HERE and that never means "global content" — the CHECK makes a subject-less row unwritable. All three invitation types audit; `auditableAssociationOrg` returns a subject for each.
...
- OV-013 could not ship before migration 0033 and that is the rule, not the history: #274 requires a type-to-confirm, a notification AND an `association_events` row of every sever, so a sever whose subject the audit table cannot hold does not ship at all. `severAssociationWithAuditStatement` serves both subjects through `subjectSql`; a sending church can only be severed FROM a network, and asking for anything else throws.
