...
- ⚖ A RE-STAMP ADDS ONE SECOND, NEVER A DAY (ruled 2026-08-21, #566). A rebase that puts the newest migration's `when` at or below a sibling is repaired by `pnpm exec tsx scripts/restamp-migration.ts` — the tail moves to `max(its own, highest sibling + 1000ms)`, in the journal AND in its own header, the only two places a `when` lives; the header rewrite anchors on the OLD NUMBER, because a header also names SIBLING rows (0048 lists three, 0049 ten). Four hand re-stamps of +24h put the journal ~46h ahead of the wall clock, so the next honest `Date.now()` landed BELOW the tail; one second lets real time close the gap instead. `pnpm db:generate` runs the script, so a freshly minted collision resolves itself. Entries behind the tail are history and are never re-stamped.
