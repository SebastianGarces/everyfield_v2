...
- The `?invitation=` link has ONE spelling — `invitationRegisterPath()` in `src/lib/invitations/register-path.ts`, an IMPORT-FREE leaf so the `"use client"` copy-link button can hold it too. Never re-spell the query key, and never move it back into `email.ts`: that module builds a Resend instance at module scope, so importing it from the browser ships the SDK into the page chunk.
