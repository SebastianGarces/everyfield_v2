...
- The refusal is `UnauthorizedError`, and its `SESSION_EXPIRED_DIGEST` is the ONLY thing `(dashboard)/error.tsx` may claim sign-in expiry on — a client boundary gets no message in production, and every other 500 gets a sentence that does not diagnose.
