...
- ⚖ …BUT IT STILL RECONCILES, ONCE THE PUSH HAS COMMITTED (#527): `void action(id).then(() => whenPushCommits(leaving)).then(() => router.refresh()).catch(() => {})`, where `whenPushCommits` resolves when `location.pathname` leaves the route the click started on (3s cap). Skipping the reconcile is a bug, and #308 shipped it skipped on a FALSE premise — "the destination's own render is a fresher read". A client-side push REUSES the layout segments both routes share rather than re-rendering them (partial rendering; `.next-docs/01-app/02-guides/authentication.mdx:1350`), and the shell's bell sits in exactly such a shared layout, so nothing re-read it on arrival: 25 unread, row click, `read_at` set, bell stuck at 25 for the session (12/12 navigated, 0/12 counted, measured on the pre-#527 preview).
- …and AFTER THE ACTION is NOT the same claim as after the push: chaining the refresh on the action alone still stranded 8 of 11, because the action settles in 200–500 ms and the push commits in 180–430 ms, so they overlap. Waiting on the URL is what separates them — 22/22 navigated, 21/21 counts moved (the 22nd click landed pre-hydration and fired no action at all). The `.catch` is part of the shape: an unhandled rejection on a page the user has left has nobody to tell.
