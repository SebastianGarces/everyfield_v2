...
- ⚖ AND THE ISSUE BODY NAMES NO PERSON: the board repo is PUBLIC, so the submitter's name, their address and their church's name never reach it. The context is three uuids — feedback row, church, user — which resolve for anyone holding database access and say nothing to anyone else. The DESCRIPTION does ship verbatim, because an issue without it is not actionable, which is why the widget's own copy says the words are published.
