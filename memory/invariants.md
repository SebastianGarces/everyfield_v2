...
- A body is sanitised in exactly ONE place per surface. `MessageBody` owns both the sanitise and the merge for the message detail page; a caller that sanitised first and let the component sanitise again is the shape that corrupted that page worse than the email.
...
- ONE rule decides where the RSVP buttons go, on every surface: `parseRichEmailBody`. The COM-015 preview and `CommunicationEmail` both call it over the same sanitised HTML — a second rule in either place previews a layout the recipient never receives. The text/plain half (`parseBody` in `communication-email.tsx`) matches the token ANYWHERE on a line, because `richTextToPlainText` writes `- ` before a list item and an equality test against the bare token delivers `__EF_CONFIRM__` as literal text.
