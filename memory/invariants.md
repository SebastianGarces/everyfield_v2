...
- …and the re-stamp is `previous.when + 1000`, ONE SECOND, never plus a day — a size no mechanism can hold you to: `seat.test.ts` → "the journal never regresses" enforces THAT `when` rises and cannot enforce BY HOW MUCH, so both spellings pass CI. A day-sized bump pushes the journal past the wall clock, and every migration generated before real time catches up then lands below the tail and fails that guard through no fault of its own — 0055–0057 walked two days ahead exactly this way (#190).
