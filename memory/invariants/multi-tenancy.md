...
**WHAT THE SENDING CHURCH'S ACCEPT AND DECLINE DO NOT DO, and why.** Accept sets `sending_churches.sending_network_id` through the ordinary accept batch and decline sets the invitation's status, and that is all: there is **no `association_events` row and no milestone notification on either path**. Both tables are CHURCH-scoped by a NOT NULL `church_id` — `association_events.church_id` because its subject is a plant (see that table's own header, which asks for a subject column and a ruling before this changes), and `notifications.church_id` because it is the tenancy boundary every read filters on (N-010). A sending church joining a network names no church, so neither row has anywhere to be filed. The record it does leave is the invitation itself — `status`, `responded_by`, `responded_at` — and the network reads the answer in its own invitations list, which is where its outstanding invitations already live. Nulling either tenant column to make room for this is the change neither table permits; the subject column is.

**THIS IS AN OPEN GAP, NOT A DECISION.** #304's WS3 acceptance criterion asks for both the audit row and the milestone, so the paragraph above describes what the schema currently forbids — not what the product intends. The ruling that settles it is **#351** (which subject column shape `association_events` gets, and what a notification about a plantless event is anchored to; "org-only milestones stay off the notification rail" is one of the answers on the table). Three code sites close it when #351 lands: `auditableAssociationOrg`'s `sending_church_to_network` arm and the `if (updated.targetChurchId)` gates on the accept and decline paths, all in `src/lib/invitations/core.ts`. None of the three may be changed alone, and the reason is measurable rather than stylistic: `notifications.church_id` is depended on by 121 references across `src/lib/notifications/`, and `memory/invariants.md:27` gives a null `church_id` the repo-wide meaning "global content", so making it nullable would not be a one-line widening — it would restate the tenancy boundary for every notification read in the product.

`scripts/g3-association-lifecycle.ts` §7 is the tripwire, and it covers **all three clauses on both paths** — the audit row, the accept milestone and the decline milestone — printing each as an `UNMET #351` line in its own output before asserting the absence. Two things follow. The run's evidence states the miss itself, so a green harness can never be read as a pass of that acceptance criterion; and the day a subject column lands, all three assertions fail and have to be flipped to REQUIRE the rows deliberately. §7 answers a first invitation with **decline** and a second with **accept**, in that order, so the decline half is exercised without an out-of-band reset between the two — the reproducibility failure that blocked this issue's first build.
