...
- **AS-010 refuses tenancy moves, not coach-only accounts (#568, Sebastian 2026-09-09).** Any seat or tenancy FK remains the same `ACCOUNT_NOT_INVITABLE_MESSAGE`; an account with neither can accept its first seat while signed in. The emailed path offers registration or sign-in based on the viewer's session, never on account lookup, and acceptance preserves coaching assignments.
...
- **A SEAT RESEND'S PROVIDER KEY IS ITS ROTATION, NEVER THE 60-SECOND BUCKET.** The org suffix is safe next door because the credential is the row's uuid and never changes; here every resend mints a new token, so two resends in one bucket presented ONE key, the provider returned the cached response, `sent: true` came back, no second message left — and the row held the digest of a link nobody had. The invitee's live link was dead, `/register` says nothing about an unknown token, and they would have registered as a cold planter at an address AS-010 then refuses to re-invite. `resendDedupeWindowAt` owns the CLIENT cooldown only. Residual: two Admins seconds apart now send two emails, each with a live-then-superseded link, which is what the message already tells the reader.
- **A refused seat resend restores the previous digest.** The rotation commits before the send is proven, so without the restore a provider outage would kill the link the invitee already holds and deliver nothing to replace it — every retry destroying another. The restore is a compare-and-set on the digest that call wrote.
- **`describeSeatInvitationForRegistration` throws on a database failure and must keep doing so.** A blanket catch there can only swallow a database error, and `register` reads the resulting null as "no invitation": the account is created as a cold planter with `seat: "owner"`, no plant, no person link and the invitation still pending. `users_email_unique` then holds the address, so the visitor cannot retry and cannot be re-invited. A loud failure is repeatable; a wrong write is not.
- **Registration single use is `users_email_unique`; signed-in acceptance locks the account and claims the token.** The claim is a compare-and-set on `pending` batched with the account, but the token is bound to one address, so at most one account can ever be created for it. **Accepted residual:** a revoke committing between the register GET and that batch leaves an account holding the seat while the row reads `revoked`; closing it needs the users insert itself to be conditional on the invitation.
