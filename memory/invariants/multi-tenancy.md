...
- **A row the pairing rejects is SEEN first, then excluded in TypeScript.** The `WHERE` is widened to the FK alone (`oversightReachCondition`) and the pairing is applied per row (`classifyOversightCandidate`), because an exclusion that happens inside SQL is invisible: nothing could count a `users` row provisioned with an oversight FK its role cannot use. A widened SELECT is not a widened audience — every row still passes the same pairing before `enqueue` sees it — and that probe returns `false` for the empty org rather than `SQL | undefined`, so it carries no `and()` fail-open of its own. Both encodings of the one pairing live in `oversight-audience.ts` and are tied by a test over `OVERSIGHT_ADMIN`'s rows, since a `WHERE` cannot call a TypeScript predicate.
- **And the two populations come back PARTITIONED, not flagged.** An optional field made one type mean "recipient OR defect", so a consumer that skipped the check mailed a cross-paired admin another tenant's milestone — and it compiled. Split arrays (`OversightAudience`) make the guard the SHAPE, not a `continue` every future caller must remember.
