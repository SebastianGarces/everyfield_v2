...
Every formatter in `datetime.ts` takes a `timeZone` argument (default `APP_TIME_ZONE` = UTC). Church-scoped surfaces plumb the church's IANA zone down from the `churches.time_zone` row — never a process global, never a module-level church pointer. `APP_TIME_ZONE` remains the pin for meeting wall-clocks (a naive `datetime-local` stored as UTC so "7:00 PM in" is "7:00 PM out") and for any surface that has not been handed a church.
