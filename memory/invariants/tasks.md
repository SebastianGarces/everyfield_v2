...
The cookie survived that ruling as "a fast path and nothing more", and #411 deleted it — the item PR #393 parked, decided on the evidence rather than deferred again.

**It never bought the thing its name claimed.** `getLatestPhaseTransition` reads the answer in the SAME LEFT JOIN as the transition — the answer is a fact about that transition, and fetching it separately would let the plant move in between and pair a new transition with an old answer — so the query the cookie was supposed to save is the query the prompt cannot render without. Zero round trips avoided, zero latency.

What it cost was a second, weaker source of "already answered": a year-long value, held by a browser, about an answer that belongs to the PLANT, read in `buildPhaseTemplatePrompt` without the row beside it, with no un-answer path anywhere in the product. The asymmetry ("a cookie can only ever *suppress* a prompt, never restore one") made that survivable, not free — it left exactly one failure mode, a browser permanently hiding a prompt the database says is open, and the whole defence against it was a rule about WHICH presses may mint. That rule needed a field of its own on both decision types (`fastPathTransitionId`, distinct from `answeredTransitionId`), and it shipped WRONG on the dismiss side and had to be fixed in a later round. A guard rail that intricate, protecting a saving of nothing, is the shape a sweep exists to remove.

So: `PHASE_TEMPLATE_PROMPT_COOKIE` and its `MAX_AGE`, `markPromptAnswered`, the cookie read in the loader, the second parameter of `buildPhaseTemplatePrompt` / `readPhaseTemplatePrompt` / `getPhaseTemplatePrompt`, `fastPathTransitionId` on both decisions, and `DeclinePhaseTemplatePromptResult.status` — which existed only so the cookie mint could tell a press that WROTE its row from one that found somebody else's — are all gone. The row is the whole mechanism. The one cookie this feature still writes is the partial-import receipt below, which carries no answer and expires in two minutes.
...
2. `importPhaseTemplatesAction` writes it to **`PHASE_TEMPLATE_RECEIPT_COOKIE`**, a flash cookie with a two-minute `maxAge`, **before** the `refresh()` that re-reads the route. Since #411 it is the only cookie this module writes, and it is its own trigger as well as its payload: setting a cookie in a Server Action re-renders the page by itself, and `refresh()` follows.
...
**There is ONE transition id in a decision now, and that is the point.** It used to be two: `answeredTransitionId` ("the route changed", drives `refresh()`) beside `fastPathTransitionId` ("this press owns a row nothing will take away", the only thing allowed to mint the browser cookie). They differed on exactly one input, `already_answered`, and they differed on it for BOTH buttons — Import shipped the split first, and "Not now" was minting off `answeredTransitionId` until 2026-08-12, because `declinePhaseTemplatePrompt` returned the transition id whether or not its `ON CONFLICT DO NOTHING` had written. The harm was the same on either path: a decline racing an Import claims nothing, the Import's first `importTaskTemplate` throws, `acceptPhaseTemplatePrompt` RELEASES the claim, and the declining browser is left holding a 365-day cookie that suppresses — before the row is ever read — a prompt the row says is unanswered, with no un-answer path, forever. #411 removed the cookie rather than the bug's latest instance, so the second field and the `declined` / `already_answered` distinction that fed it are both gone. Re-adding either means re-adding a reader for it, and the only reader it ever had is the thing that was deleted.
...
## What `/tasks` reads off the URL, and off the clock (#411)
...
Three defects of one family — a value taken on trust — found by the structural sweep, and worth recording because none of them is visible from the code that suffers.

**The filters were cast, and the route has no error boundary.** `?status=`, `?priority=` and `?category=` were read as `[param].flat() as TaskStatus[]`. That array goes into `inArray(tasks.status, …)`; `tasks.status` is a `varchar` under a CHECK over four values, so an unrecognised string reached Postgres, was refused, and rendered `/tasks`'s own failure — a typo in a shared link, or a bookmark predating a renamed value, was a broken page. `parseTaskListSearchParams` parses each one through the write schemas' own zod enums and DROPS what it does not recognise, which is the part worth stating: a filter is a view of a list, so the honest answer to a status that does not exist is the unfiltered list, not an error.

**The related-person note was unscoped.** `/tasks` pre-loads a follow-up note for each person-related task, keyed by `tasks.related_id` — and that column holds whatever the create action was given (`relatedType: "person"` plus any uuid). `getLatestPersonNote` had no church scope, and `person_activities` carries no `church_id` of its own, so the note read has to JOIN `persons` to get one. Until it did, a task created against another tenant's person printed that person's latest note on the card. The general rule: `related_id` is client input, and a read through it is a read a client aimed.

**The due-date arithmetic read the runtime's clock, twice.** `getDueDateInfo` floored `new Date()` with `setHours(0,0,0,0)` and parsed the due date as local midnight; `groupTasksByDueDate` did the same and then called `toISOString()`, which converts to UTC — so on a server east of UTC the "today" string was YESTERDAY and every task sat in the wrong bucket. The card is server-rendered and then hydrated, so the first also meant the server and the browser disagreed about "Due today" for several hours a day, on every row (React #418). Both now take `now` as a parameter and compare through `relativeDayOffset` in `APP_TIME_ZONE`; `/tasks` reads the clock ONCE and hands the same instant to the group headings and to every card.

**And the parameter is REQUIRED, because the exemption went stale inside the pass that wrote it.** `getDueDateInfo` and `TaskCardViewProps.now` first shipped with a clock default, justified by "a surface that never hydrates (the marketing vignettes) has no second render to disagree with". That left the whole rule one omitted prop away in a component that hydrates, with no type error and no test to catch it — and the one caller taking the exemption was already wrong: `task-followups-fixture.ts` built its `YYYY-MM-DD` from `getFullYear()/getMonth()/getDate()`, the RUNTIME's calendar, under a comment asserting that was what `getDueDateInfo` compared against. After the move it compared in `APP_TIME_ZONE`, so fixture and reader were on two calendars with a comment claiming one. Nothing failed only because `/` prerenders on a UTC builder. Both are now required with no default; the vignette takes ONE `new Date()` per render and hands it to `tasksFixture(now)` and to every `TaskCardView`, and the fixture builds its dates with `addCalendarDays` — the same arithmetic the reader measures with. A convention that a comment has to carry is a convention that goes stale; make it the compiler's.

**One `as` was left, in the module written to abolish casts.** `parseEnumParam` took a hand-written structural stand-in for the zod interface (`{ safeParse(value: unknown): { success: boolean; data?: unknown } }`), which discards the parsed type, so the survivors were asserted `as T[]` and `T` came from the CALL SITE — `parseEnumParam<TaskPriority>(params.status, taskStatusSchema)` compiled and silently mistyped the filter. It is now `schema: z.ZodType<T>`, `safeParse`'s own `data` is threaded through, and the three callers infer `T` instead of promising it. The honest type was free; zod is already the dependency the enums come from.
