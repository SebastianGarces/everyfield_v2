...
| Create invitation | `src/lib/invitations/service.ts:createInvitation({inviteeEmail, inviteAs?})` | Oversight admin action — inviting org + `type` derived from the session. A client names an EMAIL, never a target id: there is no plant picker (it would list every plant in the product to every org), so `resolveInvitationTarget` resolves the address against `users` server-side → an existing planter's church / sending church, or an OPEN invitation with no target. Role is settled BEFORE that lookup (account-enumeration oracle). **Refuses when the target's oversight slot is held** — `SLOT_TAKEN_MESSAGE` / `ALREADY_OURS_MESSAGE`, ruled 2026-08-03, service-layer so a forged call hits it. Expiry is `INVITATION_EXPIRY_DAYS` (30, server-fixed, ruled 2026-08-03 — no client parameter, and #23's create form has no expiry field) |
...
| Revoke invitation | `src/lib/invitations/service.ts:revokeInvitation(id)` | Original inviter (enforced in the UPDATE). The list shows Revoke only on rows the caller sent — `canRevoke` is decided on the SERVER from `inviterUserId`, which never reaches the browser |
| Redeem an invite link | `(auth)/register/page.tsx` (`?invitation=<id>`) → `register-form.tsx` (`name="invitationId"`) → `register/actions.ts` | #23. The action always read `invitationId`; nothing rendered it, so invite-at-registration was DEAD. Only an OPEN invitation is redeemable: the org is created unbound, then `bindOpenInvitationTarget` (compare-and-set on pending + both targets null + unexpired — this is what makes a link single-use) points the invitation at it, then the ordinary `acceptInvitationAs` binds. **Bind before accept**: a crash leaves pending+unbound (recoverable), never accepted+unbound |
