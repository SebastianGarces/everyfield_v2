...
| Accept invitation | `src/lib/invitations/service.ts:acceptInvitation(id)` | Target plant's planter / target sending church's admin. Binds a free slot or re-binds its own; **never replaces** another org's — refused with `ALREADY_ASSOCIATED_MESSAGE`, nothing written (`memory/invariants.md` → Multi-Tenancy) |
