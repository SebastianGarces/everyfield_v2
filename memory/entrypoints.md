...
| Flow | Entrypoint | Trigger |
|------|-----------|---------|
| Invitations surface | `(dash)/oversight/invitations/page.tsx` + `actions.ts` | Route `/oversight/invitations` (#23, OV-003). Create / pending list / revoke. The page reads `getInvitationsForOrg(actor)` — scoped to the caller's OWN org id, so there is no field or param naming an organization. `actions.ts` imports only `service.ts`, never `core.ts`, so it needs no `CORE_REACHING_ACTION_MODULES` entry |
| Create invitation | `src/lib/invitations/service.ts:createInvitation({inviteeEmail, inviteAs?})` | Oversight admin action — inviting org + `type` derived from the session. A client names an EMAIL, never a target id: there is no plant picker (it would list every plant in the product to every org), so `resolveInvitationTarget` resolves the address against `users` server-side. **An address that already has an account is REFUSED** (`ACCOUNT_EXISTS_MESSAGE`, ruled 2026-08-04 — nowhere to answer from until #277), so every invitation created today is an OPEN one. Role is settled BEFORE that lookup (account-enumeration oracle). **Also refuses when the target's oversight slot is held** — `SLOT_TAKEN_MESSAGE` / `ALREADY_OURS_MESSAGE`, ruled 2026-08-03, service-layer so a forged call hits both. Expiry is `INVITATION_EXPIRY_DAYS` (30, server-fixed, ruled 2026-08-03 — no client parameter, and #23's create form has no expiry field) |
...
| Revoke invitation | `src/lib/invitations/service.ts:revokeInvitation(id)` | ANY admin of the inviting org (ruled 2026-08-04), enforced in the UPDATE via `invitingOrgOf(actor)` — the same predicate the org-scoped list is read with, so every pending row on the screen is actionable. `inviter_user_id` is no longer in the WHERE and no longer reaches the browser as a `canRevoke` prop |
| Redeem an invite link | `(auth)/register/page.tsx` (`?invitation=<id>`) → `register-form.tsx` (`name="invitationId"`) → `register/actions.ts` | #23. The action always read `invitationId`; nothing rendered it, so invite-at-registration was DEAD. **The token is bound to the invited email** (ruled 2026-08-04): `registrationEmailMatchesInvitation` refuses a mismatch before the beta gate and before any account exists, and `hasValidInvitationBypass(id, email)` carries the same rule, so a forwarded link is neither an association nor a beta bypass for anyone else. Only an OPEN invitation is redeemable: the org is created unbound, then `bindOpenInvitationTarget` (compare-and-set on pending + both targets null + unexpired — this is what makes a link single-use) points the invitation at it, then the ordinary `acceptInvitationAs` binds. **Bind before accept**: a crash leaves pending+unbound (recoverable), never accepted+unbound |
...
**Key deps:** `organization_invitations`, `churches`, `sending_churches`, `users` tables
...
| Invitations | `(dash)/oversight/invitations/page.tsx` | Route `/oversight/invitations` (#23) — see Invitations / Associations above. Nav item un-hidden in the same change that added the page; `/oversight/plants`, `/oversight/settings`, `/oversight/sending-churches` stay hidden (#260) |
