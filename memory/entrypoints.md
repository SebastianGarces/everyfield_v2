...
**The oversight model (N-025/N-026, ruled 2026-07-27, #224):** an oversight recipient gets a daily activity DIGEST (counts only, and only on a day with activity) plus three MILESTONES (invitation accepted, stage advanced, launch date set/changed) — and never a granular per-event category, sharing on or off. `enqueue` is the single gatekeeper: `OVERSIGHT_ELIGIBLE_CATEGORIES` (`milestones` + `digest`) is checked before the toggle, and `share_activity_with_oversight` is read per call, so a flip lands at the next enqueue. `oversight.ts` composes and fans out; it deliberately reads no privacy setting, so there is one place to forget the gate and it is not there. Milestone emitters live at their sources; the digest counts through `summarizeChurchActivity()`, whose return type has four numeric fields and nothing that could hold a name.
