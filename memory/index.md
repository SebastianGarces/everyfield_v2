...
Size budget, enforced by test: `invariants.md` ≤ 64 KB, the whole tree ≤ 181 KB (re-pinned 2026-08-15 for the `email_suppressions` (#324) and `meeting_responses` (#98) contracts, after a compression pass on `contracts/db.md` — the cap governs rule LENGTH, never rule count). Maintenance is part of the REVIEWED gate — see `ops/agent-os/dod.md` § Memory.
