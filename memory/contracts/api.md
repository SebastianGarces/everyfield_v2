...
Shape pinned by `src/lib/invitations/service.test.ts`: the export surface is read off the IMPORTED module (`Object.keys`), so `export default`, `export {…} from` and arrow-const exports are all caught; every action is called with no session and must reject; no actor parameter; `core.ts` has no directive; no `"use server"` module imports it or republishes it through any chain of re-exports (closure walk, barrels included); no client component can reach it.
