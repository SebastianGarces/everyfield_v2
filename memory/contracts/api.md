...
| `GET /api/notifications/unsubscribe?token=` | **None** — sealed token in the query string | F11 opt-out (N-007). The ONE unauthenticated mutation: disables `(token.user, token.category, email)` and 303-redirects to `/unsubscribe?token=`. Reads no user/category/channel from the request — all three come from the token. Every refusal (forged/edited/expired/deleted user) redirects to the same tokenless page |
