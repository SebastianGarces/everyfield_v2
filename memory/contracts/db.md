...
- New ordinary Communication Hub sends store resolved `pastor_name` and `launch_date` in `communications.subject`, `body_html` and its flattened `body`; recipient and meeting tokens retain their established rendering behavior. Shared history readers must not resolve these plant facts from today's records, because older and Evri-generated messages used empty values. Resending a new ordinary message retains its stored pastor/date facts; a referenced Owner name containing recognized merge syntax is refused before persistence to prevent a second substitution pass.
