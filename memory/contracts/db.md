...
- New ordinary Communication Hub sends store resolved `pastor_name` and `launch_date` in `communications.subject`, `body_html` and its flattened `body`; recipient and meeting tokens retain their established rendering behavior. Shared history readers must not resolve these plant facts from today's records, because older and Evri-generated messages used empty values. Resends retain stored pastor/date literals and resolve any remaining legacy plant tokens to their originally empty values before the ordinary sender runs. Referenced Owner values containing braces, or substitutions that assemble another merge token across template boundaries, are refused before persistence to prevent a second substitution pass.
